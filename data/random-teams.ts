...
import { RANDOM_SETS } from './random-sets';
...
const SpeedSetup = ['agility', 'autotomize', 'rockpolish'];
...
export class RandomTeams {
  ...
  constructor(format: Format | string, prng: PRNG | PRNGSeed | null) {
    ...
    this.moveEnforcementCheckers = {
      ...
      Dragon: (movePool, moves, abilities, types, counter) =>
        !counter.get('Dragon') && !movePool.includes('dualwingbeat'),
    ...
    for (const moveid of moves) {
      ...
      if (move.basePower > 30 || move.multihit || move.basePowerCallback) {
        if (
          ...
          (priorityPokemon.includes(species.id) && move.priority > 0)
        ) {
  ...
  cullMovePool(
    ...
    this.incompatibleMoves(moves, movePool, statusMoves, [
      'healingwish',
      'switcheroo',
      'trick',
    ]);
    ...
    this.incompatibleMoves(moves, movePool, Setup, [
      'defog',
      'nuzzle',
      'toxic',
      'waterspout',
    ...
    this.incompatibleMoves(moves, movePool, SpeedSetup, [
      'aquajet',
      'rest',
      'trickroom',
    ]);
    ...
    this.incompatibleMoves(moves, movePool, 'switcheroo', [
      'fakeout',
      'superfang',
    ]);
    // Beartic
  ...
  randomMoveset(
    ...
    if (
      role === 'Bulky Attacker' ||
      role === 'Bulky Setup' ||
      priorityPokemon.includes(species.id)
    ) {
    ...
    for (const type of types) {
      ...
      for (const moveid of movePool) {
        ...
        if (
          ...
          (!this.noStab.includes(moveid) ||
            (abilities.has('Technician') && moveid === 'machpunch'))
        ) {
    ...
    if (
      ...
      role !== 'Bulky Support' &&
      species.baseSpecies !== 'Dudunsparce'
    ) {
      const stabMoves: string[] = [];
      for (const moveid of movePool) {
        const move = this.dex.moves.get(moveid);
    ...
    if (role.includes('Setup') || role === 'Tera Blast user') {
      ...
      const nonSpeedSetupMoves = movePool.filter(
        moveid => Setup.includes(moveid) && !SpeedSetup.includes(moveid)
      );
  ...
  shouldCullAbility(
    ...
    switch (ability) {
      ...
      case 'Flash Fire':
        return (
          ['Flame Body', 'Intimidate', 'Rock Head', 'Weak Armor'].some(m =>
            abilities.has(m)
      ...
      case 'Sheer Force':
        const braviaryCase =
          species.id === 'braviaryhisui' && role === 'Wallbreaker';
        const abilitiesCase =
  ...
  getAbility(
    ...
    if (
      abilities.has('Guts') &&
      (moves.has('facade') || moves.has('sleeptalk'))
    )
      return 'Guts';
  ...
  getDoublesItem(
    ...
    if (
      ...
      ['fakeout', 'feint', 'rapidspin', 'suckerpunch'].every(
        m => !moves.has(m)
      ) &&
    ...
    if (
      (defensiveStatTotal < 250 && ability === 'Regenerator') ||
    ...
    if (
      ...
      return ability === 'Defeatist' || defensiveStatTotal >= 275
        ? 'Sitrus Berry'
        : 'Life Orb';
  }
  ...
  getItem(
    ...
    if (
      ...
      this.dex.getEffectiveness('Ground', species) < 1
    )
      return 'Weakness Policy';
    ...
    if (
      ((!teamDetails.defog && !teamDetails.rapidSpin) ||
        (!counter.get('setup') && role !== 'Wallbreaker')) &&
    ...
    if (
      ...
      (ability === 'Regenerator' &&
        ...
        species.baseStats.def >= 110 &&
        this.randomChance(1, 3))
    )
      return 'Rocky Helmet';
    if (
      role === 'Fast Support' &&
    ...
    if (role === 'Fast Support' || role === 'Fast Bulky Setup') {
      return counter.damagingMoves.size >= 3 && !moves.has('nuzzle')
        ? 'Life Orb'
        : 'Leftovers';
    }
    if (
  ...
  randomSets = RANDOM_SETS;
  randomDoublesSets = RANDOM_SETS; // Doubles sets are the same as singles for now
  ...
  randomTeam() {
    ...
    while (baseSpeciesPool.length && pokemon.length < this.maxTeamSize) {
      ...
      if (
        ...
        pokemon.length >= this.maxTeamSize - 1 &&
        (this.getLevel(species, isDoubles) < 76 ||
          this.getLevel(species, isDoubles) > 94) &&
        !this.adjustLevel
      ) {
        continue;
      }
      ...
      if (
        ...
        (this.getLevel(species, isDoubles) < 72 ||
          this.getLevel(species, isDoubles) > 86) &&
        !this.adjustLevel
      ) {
        continue;
      }
      ...
      if (set.moves.includes('defog') || set.moves.includes('tidyup'))
        teamDetails.defog = 1;
  ...
  randomCCTeam(): RandomTeamsTypes.RandomSet[] {
    ...
    const randomN: any[] = this.randomNPokemon(
      this.maxTeamSize,
      this.forceMonotype,
      undefined,
      undefined,
    ...
    for (let forme of randomN) {
      ...
      if (forme === 'Smeargle') {
        ...
        if (learnset && learnsetSpecies === species && species.changesFrom) {
          learnset = this.dex.species.getFullLearnset(
          ...
          for (const moveid in learnset) {
            if (
              ...
              learnset[moveid].some(
                source => source.startsWith(String(this.gen)) && !evoRegion
              )
            ) {
              pool.push(moveid);
            }
      ...
      for (const species of speciesPool) {
        ...
        if (requireMoves) {
          const hasMovesInCurrentGen = Object.values(
            this.dex.species.getFullLearnset(species.id) || {}
