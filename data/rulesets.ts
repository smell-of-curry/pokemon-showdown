...
export const Rulesets: import("../sim/dex-formats").FormatDataTable = {
    ...
    onValidateTeam(team) {
      ...
      for (const [itemid, num] of itemTable) {
        ...
        return [
          ...
          `(You have more than ${itemLimit} ${
            this.dex.items.get(itemid).name
          })`,
        ];
      }
    },
      ...
      for (const set of team) {
        ...
        if (passableBoosts) {
          return [
            `${
              set.name || set.species
            } has Baton Pass and a way to boost its stats or pass trapping, which is banned by Baton Pass Stat Trap Clause.`,
          ];
        }
      }
    },
  ...
  stabmonsmovelegality: {
    ...
    checkCanLearn(move, species, setSources, set) {
      const nonstandard =
        move.isNonstandard === "Past" && !this.ruleTable.has("standardnatdex");
      if (
        !nonstandard &&
        !move.isZ &&
        !move.isMax &&
  ...
  crazyhouserule: {
    ...
    onFaint(target, source, effect) {
      ...
      const newPoke = source.side.addPokemon({
        ...target.pokemonSet,
        item: target.item,
      })!;
  ...
  chimera1v1rule: {
    ...
    onBeforeSwitchIn(pokemon) {
      ...
      pokemon.pokemonSet.evs = allies[2].pokemonSet.evs;
      pokemon.pokemonSet.nature = allies[2].pokemonSet.nature;
      pokemon.pokemonSet.ivs = allies[2].pokemonSet.ivs;
  ...
  tiershiftmod: {
    ...
    onModifySpecies(species, target, source, effect) {
      ...
      if (target) {
        if (this.toID(target.pokemonSet.item) === "lightclay") tier = "rubl";
        if (this.toID(target.pokemonSet.item) === "damprock") tier = "publ";
        if (this.toID(target.pokemonSet.item) === "heatrock") tier = "publ";
      }
    ...
    onModifySpecies(species, target) {
      ...
      if (!newSpecies.prevo) {
        ...
        for (statid in prevoSpecies.baseStats) {
          const change =
            baseSpecies.baseStats[statid] - prevoSpecies.baseStats[statid];
          const formeChange =
        ...
        case "doomdesire": {
          move.onTry = function (source, subtarget) {
            if (!subtarget.side.addSlotCondition(subtarget, "futuremove"))
              return false;
            Object.assign(
              subtarget.side.slotConditions[subtarget.position]["futuremove"],
              {
                move: "doomdesire",
                source: source,
                moveData: {
  ...
  godlygiftmod: {
    ...
    onModifySpecies(species, target, source) {
      ...
      const stat = Dex.stats.ids()[target.side.team.indexOf(target.pokemonSet)];
      const newSpecies = this.dex.deepClone(species);
  ...
  franticfusionsmod: {
    ...
    onModifySpecies(species, target, source, effect) {
      if (!target) return;
      const newSpecies = this.dex.deepClone(species);
      const fusionName = target.pokemonSet.name;
  ...
  uniquepokemoves: {
    ...
    onValidateTeam(team, format, teamHas) {
      ...
      for (const [moveid, num] of pokemoves) {
        ...
        problems.push(
          `You have ${num} Pok\u00e9mon with ${
            this.dex.species.get(moveid).name
          } as a Pokemove.`,
          `(Each Pok\u00e9mon can only be used as a Pokemove ${uniquePokemoves} time${
            uniquePokemoves === 1 ? "" : "s"
          } per team.)`
        );
      }
  ...
  ferventimpersonationmod: {
    ...
    onResidual(pokemon) {
      ...
      const impersonation = this.dex.species.get(pokemon.pokemonSet.name);
      if (
      ...
      pokemon.baseMaxhp = Math.floor(
        (Math.floor(
          2 * pokemon.species.baseStats["hp"] +
            pokemon.pokemonSet.ivs["hp"] +
            Math.floor(pokemon.pokemonSet.evs["hp"] / 4) +
            100
        ) *
          pokemon.level) /
          100 +
      ...
      const newMaxHP = pokemon.volatiles["dynamax"]
        ? 2 * pokemon.baseMaxhp
        : pokemon.baseMaxhp;
      pokemon.hp = this.clampIntRange(
        newMaxHP - (pokemon.maxhp - pokemon.hp),
        1,
        newMaxHP
      );
      pokemon.maxhp = newMaxHP;
      this.add("-heal", pokemon, pokemon.details, pokemon.getHealth, "[silent]");
      const oldAbilityKey: string =
        Object.keys(oldPokemon.abilities).find(
          (x) => (oldPokemon.abilities as any)[x] === oldAbilityName
        ) || "0";
      const newAbility: string =
        (impersonation.abilities as any)[oldAbilityKey] ||
        impersonation.abilities["0"];
