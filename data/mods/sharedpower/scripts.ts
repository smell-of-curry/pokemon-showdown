export const Scripts: ModdedBattleScriptsData = {
  field: {
    suppressingWeather() {
      for (const pokemon of this.battle.getAllActive()) {
        const innates = Object.keys(pokemon.volatiles).filter(x =>
        ...
        if (
          ...
          (pokemon.getAbility().suppressWeather ||
            innates.some(
              x =>
                this.battle.dex.abilities.get(x.replace('ability:', ''))
                  .suppressWeather
            ))
        ) {
          return true;
  ...
  pokemon: {
    hasAbility(ability) {
      ...
      return (
        this.ability === abilityid || !!this.volatiles['ability:' + abilityid]
      );
    },
    ignoringAbility() {
