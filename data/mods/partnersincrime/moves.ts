export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
  gastroacid: {
    ...
    condition: {
      ...
      onStart(pokemon) {
        ...
        const keys = Object.keys(pokemon.volatiles).filter(x =>
        ...
        if (
          ...
          for (const moveSlot of source.moveSlots) {
            if (moveSlot.id === move.id) {
              moveSlot.pp = 0;
              if (
                !source.m.curMoves.includes(moveSlot.id) &&
                source.m.trackPP.get(moveSlot.id)
              ) {
  ...
  skillswap: {
    ...
    onHit(target, source, move) {
      ...
      const ally = source.side.active.find(
        mon => mon && mon !== source && !mon.fainted
      );
      const foeAlly = target.side.active.find(
        mon => mon && mon !== target && !mon.fainted
      );
