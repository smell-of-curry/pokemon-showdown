export const Scripts: ModdedBattleScriptsData = {
  ...
  fieldEvent(eventid, targets) {
    ...
    for (const side of this.sides) {
      ...
      for (const active of side.active) {
        ...
        const ally = active.side.active.find(
          mon => mon && mon !== active && !mon.fainted
        );
        if (
          eventid === 'SwitchIn' &&
          ally?.m.innate &&
          targets &&
          !targets.includes(ally)
        ) {
    ...
    while (handlers.length) {
      ...
      if (
        (handler.effectHolder as Pokemon).fainted ||
        (handler.state?.pic as Pokemon)?.fainted
      )
        continue;
      if (
        eventid === 'Residual' &&
  ...
  endTurn() {
    ...
    for (const side of this.sides) {
      for (const pokemon of side.active) {
        pokemon.moveSlots = pokemon.moveSlots.filter(move =>
          pokemon.m.curMoves.includes(move.id)
        );
        ...
        if (ally) {
          ...
          allyMoves = allyMoves.filter(
            move =>
              ...
              ally.m.curMoves.includes(move.id)
          );
          for (const aMove of allyMoves) {
            aMove.pp = this.clampIntRange(
              aMove.maxpp - (pokemon.m.trackPP.get(aMove.id) || 0),
              0
            );
          }
        }
  ...
  pokemon: {
    setAbility(ability, source, isFromFormeChange) {
      ...
      const ally = this.side.active.find(
        mon => mon && mon !== this && !mon.fainted
      );
      if (ally?.m.innate) {
    ...
    deductPP(move, amount, target) {
      ...
      if (!this.m.curMoves.includes(move.id)) {
        this.m.trackPP.set(
          move.id,
          (this.m.trackPP.get(move.id) || 0) + amount
        );
      }
      return amount;
    },
