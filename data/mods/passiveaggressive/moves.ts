  ...
  spikes: {
    ...
    condition: {
      ...
      onSwitchIn(pokemon) {
        ...
        this.damage(
          (calc * damageAmounts[this.effectState.layers] * pokemon.maxhp) / 24
        );
      },
    },
  },
    ...
    condition: {
      ...
      onTryMove(pokemon, target, move) {
        if (move.type === 'Fire') {
          ...
          if (calc)
            this.damage(
              this.clampIntRange(Math.round((calc * pokemon.maxhp) / 4), 1)
            );
  ...
  saltcure: {
    ...
    condition: {
      ...
      onResidual(pokemon) {
        ...
        if (calc)
          this.damage(
            (calc * pokemon.baseMaxhp) /
              (pokemon.hasType(['Water', 'Steel']) ? 4 : 8)
          );
      },
      onEnd(pokemon) {
...
function calculate(
  ...
  const typeMod = Math.pow(
    2,
    battle.clampIntRange(pokemon.runEffectiveness(move), -6, 6)
  );
