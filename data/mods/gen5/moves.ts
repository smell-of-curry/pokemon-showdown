export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
  ...
  copycat: {
    ...
    onHit(pokemon) {
      if (
        !this.lastMove ||
        this.dex.moves.get(this.lastMove.id).flags['failcopycat']
      ) {
        return false;
      }
  ...
  electroball: {
    ...
    basePowerCallback(pokemon, target) {
      const ratio = Math.floor(
        pokemon.getStat('spe') / Math.max(1, target.getStat('spe'))
      );
  ...
  gyroball: {
    ...
    basePowerCallback(pokemon, target) {
      let power =
        Math.floor(
          (25 * target.getStat('spe')) / Math.max(1, pokemon.getStat('spe'))
        ) + 1;
  ...
  quickguard: {
    ...
    condition: {
      ...
      onTryHit(target, source, effect) {
        ...
        if (
          effect &&
          (effect.id === 'feint' || this.dex.moves.get(effect.id).priority <= 0)
        ) {
          return;
        }
