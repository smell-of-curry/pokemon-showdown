export const Conditions: import('../../../sim/dex-conditions').ModdedConditionDataTable =
  {
    ...
    frz: {
      ...
      onAfterMoveSecondary(target, source, move) {
        if (
          (move.secondary && move.secondary.status === 'brn') ||
          move.statusRoll === 'brn'
        ) {
          target.cureStatus();
        }
    ...
    lockedmove: {
      ...
      onResidual(target) {
        if (
          (target.lastMove && target.lastMove.id === 'struggle') ||
          target.status === 'slp'
        ) {
    ...
    stall: {
      ...
      onStallMove() {
        ...
        this.debug(
          'Success chance: ' +
            Math.round((counter * 1000) / 255) / 10 +
            '% (' +
            counter +
            '/255)'
        );
...
function residualdmg(battle: Battle, pokemon: Pokemon) {
  if (pokemon.volatiles['residualdmg']) {
    ...
    battle.damage(
      battle.clampIntRange(Math.floor(pokemon.maxhp / 8), 1),
      pokemon
    );
  }
}
