...
export const Rulesets: import('../../../sim/dex-formats').ModdedFormatDataTable =
  {
    ...
    nc2000movelegality: {
      ...
      onValidateSet(set) {
        const illegalCombos: {
          [speciesid: string]: { [moveid: string]: 'E' | 'L' | 'S' };
        } = {
          arbok: { crunch: 'E' },
        ...
        for (const moveid of set.moves.map(this.toID)) {
          ...
          if (legalityList) {
            const list = moveSources[moveid].filter(
              x => !x.includes(legalityList[moveid])
            );
            if (!list.length) {
          ...
          for (const id of notUsableAsTM) {
            if (
              ...
              problems.push(
                `${species.name} can't learn ${this.dex.moves.get(id).name}.`
              );
            }
          }
        }
        if (
          problems.some(x =>
            notUsableAsTM
              .map(y => this.dex.moves.get(y).name)
              .some(z => x.includes(z))
          )
        ) {
          problems.push(
