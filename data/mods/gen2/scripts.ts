...
export const Scripts: ModdedBattleScriptsData = {
  ...
  pokemon: {
    ...
    boostBy(boost) {
      ...
      for (i in boost) {
        ...
        if (
          delta > 0 &&
          this.getStat(i as StatIDExceptHP, false, true) === 999
        ) {
          delta = 0;
          continue;
        }
  ...
  actions: {
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      if (pokemon.moveThisTurn) {
        ...
        this.battle.debug(
          '' +
            pokemon.fullname +
            ' INCONSISTENT STATE, ALREADY MOVED: ' +
    ...
    tryMoveHit(target, pokemon, move) {
      ...
      let hitResult = this.battle.singleEvent(
        'PrepareHit',
        move,
        {},
        target,
      ...
      if (
        accuracy !== true &&
        accuracy !== 255 &&
        !this.battle.randomChance(accuracy, 256)
      ) {
        this.battle.attrLastMove('[miss]');
        this.battle.add('-miss', pokemon);
        damage = false;
      ...
      if (move.multihit) {
        ...
        for (i = 0; i < hits && target.hp && pokemon.hp; i++) {
          ...
          if (
            nullDamage &&
            (moveDamage || moveDamage === 0 || moveDamage === undefined)
          )
            nullDamage = false;
          damage = moveDamage || 0;
    ...
    moveHit(target, pokemon, move, moveData, isSecondary, isSelf) {
      ...
      if (target) {
        ...
        if (moveData.heal && !target.fainted) {
          const d = target.heal(
            Math.round((target.maxhp * moveData.heal[0]) / moveData.heal[1])
          );
          if (!d && d !== 0) {
            this.battle.add('-fail', target);
    ...
    getDamage(source, target, move, suppressMessages) {
      ...
      if (isCrit) damage *= 2;
      damage = Math.floor(
        this.battle.runEvent('ModifyDamage', attacker, defender, move, damage)
      );
