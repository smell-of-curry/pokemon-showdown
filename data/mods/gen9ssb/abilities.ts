...
export const Abilities: import('../../../sim/dex-abilities').ModdedAbilityDataTable =
  {
	...
	*/
    ...
    coalescence: {
      shortDesc:
        'Moves drain 37%. Allies heal 5% HP. <25% HP, moves drain 114%, allies get 10%.',
        ...
        for (const ally of pokemon.side.pokemon) {
          ...
          if (
            ally.heal(
              this.modify(
                ally.baseMaxhp,
                pokemon.hp > pokemon.maxhp / 4 ? 0.05 : 0.1
              )
            )
          ) {
            this.add(
    ...
    snowballer: {
      shortDesc:
        "This Pokemon's Attack is raised 1 stage if hit by an Ice move; Ice immunity.",
      name: 'Snowballer',
      onTryHitPriority: 1,
      onTryHit(target, source, move) {
        if (target !== source && move.type === 'Ice') {
    ...
    frozenfortuity: {
      ...
      onSwitchOut(pokemon) {
        if (
          pokemon.baseSpecies.baseSpecies !== 'Kyurem' ||
          pokemon.transformed ||
          !pokemon.hp
        )
          return;
    ...
    peerpressure: {
      shortDesc:
        'All moves used while this Pokemon is on the field consume 4 PP.',
      name: 'Peer Pressure',
      onStart(pokemon) {
    ...
    adaptiveengineering: {
      ...
      onResidual(source) {
        if (
          source === undefined ||
          source.foes() === undefined ||
          source.foes()[0] === undefined
        )
          return;
    ...
    aswesee: {
      shortDesc:
        '1x per turn: Stat gets boosted -> 50% chance to copy, 15% to raise another.',
      ...
      onFoeAfterBoost(boost, target, source, effect) {
        ...
        if (this.randomChance(1, 2)) {
          if (
            effect &&
            ['As We See', 'Mirror Herb', 'Opportunist'].includes(effect.name)
          )
            return;
