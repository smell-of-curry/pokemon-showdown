...
export const Conditions: {
  ...
  clementine: {
    ...
    onStart() {
      this.add(
        `c:|${getName('Clementine')}|Je suis peut-être con comme une table`
      );
    },
    onSwitchOut(pokemon) {
      if (pokemon.volatiles['flipped']) {
        ...
        this.add(
          `c:|${getName('Clementine')}|I fucking love air-conditioning.`
        );
      }
    },
  ...
  hecate: {
    ...
    onFaint() {
      this.add(
        `c:|${getName('Hecate')}|git checkout --detach HEAD && git commit -m "war crimes"`
      );
    },
  },
  hizo: {
  ...
  imperial: {
    ...
    onSwitchOut(pokemon) {
      ...
      if (foe) {
        this.add(
          `c:|${getName('Imperial')}|ofc u have ${foe}. bad mu as always...`
        );
      }
    },
    onFaint() {
        ...
        source.baseMaxhp = Math.floor(
          (Math.floor(
            2 * source.species.baseStats['hp'] +
              source.pokemonSet.ivs['hp'] +
              Math.floor(source.pokemonSet.evs['hp'] / 4) +
              100
          ) *
            source.level) /
            100 +
  ...
  marillvibes: {
    ...
    onSwitchOut() {
      this.add(
        `c:|${getName('marillvibes ♫')}|Here for a good time, not a long time!`
      );
    },
    onFaint() {
  ...
  twoofroses: {
    ...
    onWeatherChange(target, source, sourceEffect) {
      ...
      if (!currentWeather && !target.hasType('Dark')) {
        ...
        if (
          ['raindance', 'primordialsea'].includes(currentWeather) &&
          ...
          ['hail', 'snow'].includes(currentWeather) &&
          !target.hasType('Ice')
        ) {
          type = 'Ice';
        } else {
  ...
  bioticorbself: {
    ...
    onResidual(target, pokemon, effect) {
      ...
      this.add(
        `c:|${getName((source.illusion || source).name)}|${this.sample(quotes)}`
      );
    },
