...
export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
    ...
    onHit(target, source) {
      ...
      switch (randFormat) {
        ...
        case 'gen9computergeneratedteams':
          msg =
            "We asked an AI to make a randbats set. YOU WON'T BELIEVE WHAT IT CAME UP WITH N.N";
          break;
      }
  ...
  powerup: {
    ...
    flags: { heal: 1 },
    onModifyMove(move, source, target) {
      const fntAllies = source.side.pokemon.filter(
        ally => ally !== source && ally.fainted
      );
  ...
  meatgrinder: {
    ...
    condition: {
      ...
      onResidual(pokemon, source) {
        this.damage(
          pokemon.baseMaxhp / (pokemon.hasType(['Normal', 'Fairy']) ? 4 : 8)
        );
  ...
  shepherdofthemafiaroom: {
    ...
    onBasePower(relayVar, source, target, move) {
      if (
        source.getStat('spe', false, true) > target.getStat('spe', false, true)
      ) {
  ...
  dreameater: {
    ...
    onTryImmunity(target) {
      return (
        target.status === 'slp' || target.hasAbility(['comatose', 'mensiscage'])
      );
    },
  },
  electroshot: {
  ...
  sleeptalk: {
    ...
    onTry(source) {
      return (
        source.status === 'slp' || source.hasAbility(['comatose', 'mensiscage'])
      );
    },
  },
  snore: {
