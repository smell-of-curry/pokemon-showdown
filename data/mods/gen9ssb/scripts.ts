...
export function changeSet(
  ...
  if (oldShiny !== pokemon.pokemonSet.shiny || oldGender !== pokemon.gender)
    context.add('replace', pokemon, details);
  if (changeAbility)
    pokemon.setAbility(newSet.ability as string, undefined, true);
    ...
    const moveSlot = {
      ...
      pp:
        move.noPPBoosts || move.isZ
          ? Math.floor(move.pp * carryOver[slot])
          : Math.floor(move.pp * (8 / 5) * carryOver[slot]),
  ...
  actions: {
    terastallize(pokemon) {
      ...
      if (!pokemon.illusion && pokemon.name === 'Neko') {
        this.battle.add(
          `c:|Neko|Possible thermal failure if operation continues (Meow on fire ?)`
        );
      }
    ...
    runMegaEvo(pokemon) {
      ...
      if (speciesid === 'Trapinch' && pokemon.name === 'Arya') {
        this.battle.add(
          `c:|Arya|Oh yeaaaaah!!!!! Finally??!! I can finally Mega-Evolve!!! Vamossss`
        );
      }
    ...
    hitStepMoveHitLoop(targets, pokemon, move) {
      ...
      this.afterMoveSecondaryEvent(
        targetsCopy.filter(val => !!val) as Pokemon[],
        pokemon,
        move
      );
