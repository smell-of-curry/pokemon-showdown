      ...
      onDamagingHit(damage, target, source, move) {
        if (target.ability === 'mummy') {
          ...
          const possibleAbilities = [
            ...
            val =>
              !this.dex.abilities.get(val).flags['cantsuppress'] &&
              val !== 'mummy'
          );
      ...
      onSwitchIn(pokemon) {
        ...
        if (pokemon.m.innates) {
          for (const innate of pokemon.m.innates) {
            if (
              this.dex.abilities.get(innate).flags['cantsuppress'] ||
              innate === 'neutralizinggas'
            )
              continue;
