    ...
    recalculateStats() {
      ...
      for (statName in this.storedStats) {
        ...
        stat = Math.floor(
          (Math.floor(
            2 * stat +
              this.pokemonSet.ivs[statName] +
              Math.floor(this.pokemonSet.evs[statName] / 4)
          ) *
            this.level) /
            100 +
            5
        );
        this.baseStoredStats[statName] = this.storedStats[statName] =
          Math.floor(stat);
        ...
        if (this.boosts[statName] !== 0) {
          if (this.boosts[statName] >= 0) {
            this.modifyStat!(
              statName,
              [1, 1.5, 2, 2.5, 3, 3.5, 4][this.boosts[statName]]
            );
          } else {
            this.modifyStat!(
              statName,
              [100, 66, 50, 40, 33, 28, 25][-this.boosts[statName]] / 100
            );
          }
        }
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      if (
        ...
        this.battle.debug(
          '' +
            pokemon.fullname +
            ' move interrupted; movedThisTurn: ' +
            pokemon.moveThisTurn
        );
    ...
    tryMoveHit(target, pokemon, move) {
      ...
      if (accuracy !== true) {
        ...
        if (!move.ignoreAccuracy) {
          accuracy = Math.floor(
            accuracy * boostTable[pokemon.boosts.accuracy + 6]
          );
        }
        if (!move.ignoreEvasion) {
          accuracy = Math.floor(
            accuracy * boostTable[-target.boosts.evasion + 6]
          );
        }
        ...
        for (const secondary of moveData.secondaries) {
          ...
          if (!move.multihit || move.lastHit) {
            ...
            if (
              ...
              const effectChance = Math.floor(
                ((secondary.chance || 100) * 255) / 100
              );
              if (
