  ...
  counter: {
    ...
    damageCallback(pokemon, target) {
      ...
      if (!lastMoveThisTurnIsCounterable) {
        this.debug(
          `Stadium 1 Counter: last move ${lastMoveThisTurn.name} was not Counterable`
        );
        this.add('-fail', pokemon);
        return false;
      }
  ...
  rage: {
    ...
    condition: {
      ...
      onHit(target, source, move) {
        if (
          target.boosts.atk < 6 &&
          (move.category !== 'Status' || move.id === 'disable')
        ) {
          this.boost({ atk: 1 });
        }
      },
      ...
      onTryHit(target, source, move) {
        ...
        if (target.volatiles['substitute']) {
          if (move.recoil) {
            this.damage(
              this.clampIntRange(
                Math.floor((damage * move.recoil[0]) / move.recoil[1]),
                1
              ),
              source,
              target,
