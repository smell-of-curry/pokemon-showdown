...
export const Scripts: ModdedBattleScriptsData = {
  ...
  pokemon: {
    isGrounded(negateImmunity) {
      ...
      if (
        item === 'ironball' ||
        (this.volatiles['item:ironball'] && !this.ignoringItem())
      )
        return true;
      ...
      if (
        item === 'airballoon' ||
        (this.volatiles['item:airballoon'] && !this.ignoringItem())
      )
        return false;
      return true;
    },
    ...
    useItem(source, sourceEffect) {
      const hasAnyItem =
        !!this.item ||
        Object.keys(this.volatiles).some(v => v.startsWith('item:'));
      ...
      const item = sourceEffect?.id.startsWith('item:')
        ? (sourceEffect as Item)
        : this.getItem();
    ...
    eatItem(force, source, sourceEffect) {
      ...
      if (
        ...
        if (
          RESTORATIVE_BERRIES.has(
            item.id.startsWith('item:') ? (item.id.slice(5) as ID) : item.id
          )
        ) {
    ...
    setItem(item, source, effect) {
      ...
      if (item.id) {
        ...
        for (const ally of this.side.pokemon) {
          ...
          ally.m.sharedItemsUsed = ally.m.sharedItemsUsed.filter(
            (i: ID) => i !== (item as Item).id
          );
        }
      }
      return true;
