  ...
  getActionSpeed(action) {
    if (action.choice === 'move') {
      ...
      if (
        ...
        const linkedActions =
          action.linked ||
          linkedMoves.map(moveid => this.dex.getActiveMove(moveid));
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      if (move.flags['dance'] && moveDidSomething && !move.isExternal) {
        ...
        for (const dancer of dancers) {
          ...
          const dancersTarget =
            !target!.isAlly(dancer) && pokemon.isAlly(dancer)
              ? target!
              : pokemon;
      ...
      if (!midTurn) {
        if (action.choice === 'move') {
          ...
          if (
            linkedMoves.length &&
            !(
              action.pokemon.getItem().isChoice ||
              action.pokemon.hasAbility('gorillatactics')
            ) &&
            !action.zmove &&
            !action.maxMove
          ) {
            ...
            if (linkedMoves.includes(decisionMove)) {
              action.linked = linkedMoves.map(moveid =>
                this.battle.dex.getActiveMove(moveid)
              );
              const linkedOtherMove =
                action.linked[1 - linkedMoves.indexOf(decisionMove)];
