export const Scripts: ModdedBattleScriptsData = {
  ...
  init() {
    for (const i in this.data.Items) {
      ...
      if (item.megaStone) {
        this.modData('FormatsData', this.toID(item.megaStone)).isNonstandard =
          null;
      }
    }
  },
        ...
        for (const pokemon of this.getAllPokemon()) {
          ...
          if (ironHead >= 0) {
            const move = this.dex.moves.get(
              behemothMove[pokemon.getItem().name]
            );
  ...
  actions: {
    ...
    runMegaEvo(pokemon) {
      ...
      const oMegaSpecies = this.dex.species.get(
        (species as any).originalSpecies
      );
      pokemon.formeChange(species, pokemon.getItem(), true);
    ...
    terastallize(pokemon) {
      ...
      if (
        ...
        pokemon.getItem().name.endsWith('Mask')
      ) {
        type = this.dex.species.get(
          pokemon.getItem().forcedForme
        ).forceTeraType!;
      }
      ...
      if (pokemon.species.baseSpecies === 'Ogerpon') {
        ...
        if (pokemon.getItem().name.endsWith('Mask')) {
          ...
          const oSpecies = this.dex.species.get(pokemon.m.originalSpecies);
          const originalTeraSpecies = this.dex.species.get(
            (species as any).originalSpecies
          );
          pokemon.formeChange(species, pokemon.getItem(), true);
          this.battle.add(
            '-start',
            pokemon,
            originalTeraSpecies.requiredItem,
            '[silent]'
          );
          if (
            oSpecies.types.length !== pokemon.species.types.length ||
    ...
    getMixedSpecies(originalForme, formeChange, pokemon) {
      ...
      const deltas = (this as any).getFormeChangeDeltas(
        formeChangeSpecies,
        pokemon
      );
      const species = (this as any).mutateOriginalSpecies(
        originalSpecies,
        deltas
      );
      return species;
    ...
    getFormeChangeDeltas(formeChangeSpecies, pokemon) {
      ...
      const deltas: {
        ...
        heightm:
          (formeChangeSpecies.heightm * 10 - baseSpecies.heightm * 10) / 10,
    ...
    mutateOriginalSpecies(speciesOrForme, deltas) {
      ...
      species.heightm = Math.max(
        0.1,
        (species.heightm * 10 + deltas.heightm * 10) / 10
      );
