export const Abilities: import('../../../sim/dex-abilities').ModdedAbilityDataTable =
  {
      ...
      onStart(pokemon) {
        ...
        for (const target of pokemon.adjacentFoes()) {
          if (target.volatiles['substitute']) {
            ...
            this.hint(
              'In Gen 4, if U-turn breaks Substitute the incoming Intimidate does nothing.'
            );
          } else {
    ...
    liquidooze: {
      ...
      onSourceTryHeal(damage, target, source, effect) {
        ...
        if (
          canOoze.includes(effect.id) &&
          this.activeMove?.id !== 'dreameater'
        ) {
    ...
    unburden: {
      ...
      condition: {
        onModifySpe(spe, pokemon) {
          if (
            (!pokemon.item || pokemon.itemState.knockedOff) &&
