export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
  ...
  encore: {
    ...
    condition: {
      ...
      onStart(target, source) {
        const moveIndex = target.lastMove
  ...
  mimic: {
    ...
    onHit(target, source) {
      ...
      if (
        target.lastMove.flags['failmimic'] ||
        source.moves.includes(target.lastMove.id)
      ) {
        return false;
      }
  ...
  sketch: {
    ...
    onHit(target, source) {
      ...
      if (
        target.lastMove.flags['nosketch'] ||
        source.moves.includes(target.lastMove.id)
      ) {
        return false;
      }
  ...
  worryseed: {
    ...
    onTryHit(pokemon) {
      ...
      if (
        bannedAbilities.includes(pokemon.ability) ||
        pokemon.hasItem('griseousorb')
      ) {
        return false;
      }
    },
