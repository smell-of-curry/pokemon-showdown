...
export const Conditions: import('../../../sim/dex-conditions').ModdedConditionDataTable =
  {
    brn: {
      ...
      onAfterMoveSelf(pokemon) {
        ...
        if (pokemon.volatiles['residualdmg']) {
          this.hint(
            "In Gen 1, Toxic's counter is retained after Rest and applies to PSN/BRN.",
            true
          );
        }
      },
      ...
      onBeforeMove(pokemon, target) {
        ...
        if (!this.randomChance(128, 256)) {
          const damage =
            Math.floor(
              Math.floor(
                ((Math.floor((2 * pokemon.level) / 5) + 2) *
                  pokemon.getStat('atk') *
                  40) /
    ...
    invulnerability: {
      ...
      onInvulnerability(target, source, move) {
        ...
        this.add(
          '-message',
          'The foe ' + target.name + " can't be hit while invulnerable!"
        );
        return false;
      },
    },
