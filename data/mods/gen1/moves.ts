...
export const Moves: import('../../../sim/dex-moves').ModdedMoveDataTable = {
  ...
  counter: {
    ...
    damageCallback(pokemon, target) {
      ...
      const lastMove =
        target.side.lastMove && this.dex.moves.get(target.side.lastMove.id);
      ...
      const lastSelectedMove =
        target.side.lastSelectedMove &&
        this.dex.moves.get(target.side.lastSelectedMove);
  ...
  disable: {
    ...
    onTryHit(target) {
      ...
      return (
        target.moveSlots.some(ms => ms.pp > 0) &&
  ...
  leechseed: {
    ...
    condition: {
      ...
      onAfterMoveSelf(pokemon) {
        ...
        if (residualdmg)
          this.hint(
            "In Gen 1, Leech Seed's damage is affected by Toxic's counter.",
            true
          );
      ...
      onTryHit(target, source, move) {
        ...
        if (target.volatiles['substitute']) {
          if (move.recoil) {
            this.damage(
              this.clampIntRange(
                Math.floor((uncappedDamage * move.recoil[0]) / move.recoil[1]),
                1
              ),
              source,
              target,
          ...
          if (move.secondary?.volatileStatus === 'confusion') {
            ...
            if (
              ...
              this.randomChance(
                Math.ceil((secondary.chance * 256) / 100) - 1,
                256
              )
            ) {
