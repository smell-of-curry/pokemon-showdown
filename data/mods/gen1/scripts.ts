      ...
      for (i in boost) {
        ...
        if (changed) {
          ...
          if (this.boosts[i] >= 0) {
            ...
            this.modifyStat!(
              i,
              [100, 66, 50, 40, 33, 28, 25][-this.boosts[i]] / 100
            );
          }
  ...
  actions: {
    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      if (
        ...
        !this.battle.runEvent('BeforeMove', pokemon, target, move)
      ) {
      ...
      if (
        ...
        if (pokemon.volatiles['twoturnmove']) {
          ...
          pokemon.deductPP(
            pokemon.volatiles['twoturnmove'].originalMove,
            null,
            target
          );
        }
    ...
    useMove(moveOrMoveName, pokemon, options) {
      ...
      const moveResult = this.useMoveInner(moveOrMoveName, pokemon, {
        target,
        sourceEffect,
      });
    ...
    tryMoveHit(target, pokemon, move) {
        ...
        if (pokemon.volatiles['lockedmove']) {
          ...
          if (!pokemon.volatiles['lockedmove'].time) {
            ...
            pokemon.addVolatile(
              'confusion',
              pokemon,
              this.dex.conditions.get('lockedmove')
            );
          }
        } else {
      ...
      let hitResult = this.battle.runEvent(
        'Invulnerability',
        target,
        pokemon,
        move
      ...
      if (accuracy !== true) {
        ...
        if (accuracy !== true) {
          ...
          if (!move.ignoreAccuracy) {
            accuracy = Math.floor(
              accuracy * (boostTable[pokemon.boosts.accuracy + 6] / 100)
            );
          }
          if (!move.ignoreEvasion) {
            accuracy = Math.floor(
              accuracy * (boostTable[-target.boosts.evasion + 6] / 100)
            );
          }
    ...
    moveHit(target, pokemon, move, moveData, isSecondary, isSelf) {
      ...
      if (target) {
        ...
        if (!isSelf && !isSecondary) {
          ...
          hitResult = this.battle.runEvent(
            'TrySecondaryHit',
            target,
            pokemon,
            moveData
      ...
      if (target) {
        ...
        if (moveData.boosts && target.hp) {
          const willBoost = this.battle.boost(
            moveData.boosts,
            target,
            pokemon,
            move
        ...
        if (moveData.heal && !target.fainted) {
          const d = target.heal(
            Math.floor((target.maxhp * moveData.heal[0]) / moveData.heal[1])
          );
          if (!d) {
            this.battle.add('-fail', target);
            return false;
        ...
        if (moveData.pseudoWeather) {
          if (
            this.battle.field.addPseudoWeather(
      ...
      if (pokemon.volatiles['partialtrappinglock']) {
        pokemon.volatiles['partialtrappinglock'].damage =
          this.battle.lastDamage;
      }
      ...
      if (move.selfSwitch && pokemon.hp) {
        pokemon.switchFlag =
          move.selfSwitch === true ? true : this.dex.toID(move.selfSwitch);
      }
    ...
    getDamage(source, target, move, suppressMessages) {
      ...
      if (!isCrit) {
        ...
        let critChance = Math.floor(
          this.dex.species.get(source.pokemonSet.species).baseStats['spe'] / 2
        );
      ...
      const atkType: StatIDExceptHP =
        move.overrideOffensiveStat || (isPhysical ? 'atk' : 'spa');
      const defType: StatIDExceptHP =
