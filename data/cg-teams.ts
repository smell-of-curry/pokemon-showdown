...
export default class TeamGenerator {
    ...
    for (const i of this.itemPool) {
      if (i.itemUser && !i.isNonstandard) {
        for (const user of i.itemUser) {
          if (Dex.species.get(user).requiredItems?.[0] !== i.name)
  ...
  getTeam(): PokemonSet[] {
    let speciesPool = this.dex.species.all().filter(s => {
      ...
      if (
        s.battleOnly &&
        (!s.requiredItems?.length || s.name.endsWith('-Tera'))
      )
        return false;
  ...
  protected makeSet(species: Species, teamStats: TeamStats): PokemonSet {
    ...
    while (moves.length < 4 && movePool.length) {
        ...
        if (!isRound2) {
          ...
          for (const move of moves) {
            originalWeights.push(
              interimMovePool.find(m => m.move === move.id)!
            );
          }
      ...
      if (TeamGenerator.moveIsHazard(moves[moves.length - 1])) {
        teamStats.hazardSetters[moveID] =
          (teamStats.hazardSetters[moveID] || 0) + 1;
        movesStats.hazards++;
      }
      if (
        ['defog', 'courtchange', 'tidyup', 'rapidspin', 'mortalspin'].includes(
          moveID
        )
      )
          ...
          for (const stat in boosts) {
            const chance = Math.min(
              ...
              move.secondary?.chance ||
                100 * (ability === 'Serene Grace' ? 2 : 1)
            );
            const boost =
              ((boosts[stat as StatIDExceptHP] || 0) * chance) / 100;
            if (boost) {
    ...
    const nonStatusMoves = moves.filter(
      m => this.dex.moves.get(m).category !== 'Status'
    );
    ...
    const ivs: PokemonSet['ivs'] = {
      hp: 31,
      atk: moves.some(move => this.dex.moves.get(move).category === 'Physical')
        ? 31
        : 0,
      def: 31,
      spa: 31,
    ...
    if (species.forceTeraType) {
      ...
      hasTeraBlast &&
      ability === 'Contrary' &&
      this.prng.randomChance(2, 3)
    ) {
      teraType = 'Stellar';
    } else {
      let types = nonStatusMoves.map(m =>
  ...
  protected getMoveWeight(
    ...
    if (move.category === 'Status') {
      ...
      if (
        TeamGenerator.moveIsHazard(move) &&
        (teamStats.hazardSetters[move.id] || 0) < 1
      ) {
      ...
      if (
        ['defog', 'courtchange', 'tidyup'].includes(move.id) &&
        !teamStats.hazardRemovers
      ) {
        weight *= 32;
        ...
        weight *= Math.pow(
          0.8,
          Object.values(teamStats.hazardSetters).reduce(
            (total, num) => total + num,
            0
          )
        );
      ...
      if (move.id === 'focusenergy' && ability !== 'Super Luck') {
        const highCritMoves = movesSoFar.filter(
          m => m.critRatio && m.critRatio > 1
        );
      ...
      if (!movesStats.stallingMoves) {
        if (
          adjustedStats.def >= 80 ||
          adjustedStats.spd >= 80 ||
          adjustedStats.hp >= 80
        ) {
      ...
      if (
        ['sleeptalk', 'rest'].includes(move.id) &&
        sleepImmunities.includes(ability)
      )
        return 0;
    ...
    if (
      WEIGHT_BASED_MOVES.includes(move.id) ||
      TARGET_HP_BASED_MOVES.includes(move.id)
    )
      basePower = 60;
    /** A value from 0 to 1, where 0 is the fastest and 1 is the slowest */
    const slownessRating =
      Math.max(0, TOP_SPEED - adjustedStats.spe) / TOP_SPEED;
    ...
    let baseStat =
      move.category === 'Physical' ? adjustedStats.atk : adjustedStats.spa;
    ...
    let accuracy =
      move.accuracy === true || ability === 'No Guard' ? 110 : move.accuracy;
    if (accuracy < 100) {
    ...
    if (species.requiredItems) {
      const item: Item & EventMethods = this.dex.items.get(
        this.specialItems[species.name]
      );
      if (
        item.onBasePower &&
        (species.types.includes(moveType) || item.name.endsWith('Mask'))
      )
        powerEstimate *= 1.2;
    ...
    if (
      ...
      !['bodypress', 'foulplay'].includes(move.id)
    ) {
      powerEstimate *=
        Math.max(0.5, 1 + physicalSetup) / Math.max(0.5, 1 + specialSetup);
    }
    if (move.category === 'Special')
      powerEstimate *=
        Math.max(0.5, 1 + specialSetup) / Math.max(0.5, 1 + physicalSetup);

    const abilityBonus =
      ((ABILITY_MOVE_BONUSES[this.dex.toID(ability)] || {})[move.id] || 1) *
      ((ABILITY_MOVE_TYPE_BONUSES[this.dex.toID(ability)] || {})[moveType] ||
        1);
        ...
        for (const secondary of secondaries) {
          if (secondary.status) {
            ...
            if (
              ...
              ['psn', 'tox'].includes(secondary.status)
            ) {
              weight *= TeamGenerator.statusWeight(
                'confusion',
                secondaryChance
    ...
    weight *=
      1 +
      [0, 1 / 24, 1 / 8, 1 / 2, 1][critRate] * (ability === 'Sniper' ? 1 : 0.5);
    ...
    if (
      move.id === 'terablast' &&
      (species.baseSpecies === 'Oricorio' || species.forceTeraType)
    )
      weight *= 0.5;
  ...
  protected static moveIsPhysical(move: Move, species: Species) {
    if (move.category === 'Physical') {
      ...
      ['terablast', 'terastarstorm', 'photongeyser', 'shellsidearm'].includes(
        move.id
      )
    ) {
   ...
   */
  protected static statusWeight(
    status: string,
    chance = 1,
    slownessRating?: number
  ): number {
    if (chance !== 1)
      ...
      move.secondary && ability !== 'Sheer Force'
        ? Math.min(
            ((move.secondary.chance || 100) *
              (ability === 'Serene Grace' ? 2 : 1)) /
              100,
            100
          ) * accuracy
        : 0;
    const abilityMod =
      ability === 'Simple' ? 2 : ability === 'Contrary' ? -1 : 1;
  ...
  protected getItemWeight(
    ...
    switch (item.id) {
      // Choice Items
      case 'choiceband':
        return moves.every(x => TeamGenerator.moveIsPhysical(x, species))
          ? 50
          : 0;
      case 'choicespecs':
        return moves.every(x => TeamGenerator.moveIsSpecial(x, species))
          ? 50
          : 0;
      case 'choicescarf':
        if (
          moves.some(
            x => x.category === 'Status' || x.secondary?.self?.boosts?.spe
          )
        )
          return 0;
      ...
      case 'lifeorb':
        return (
          moves.filter(
            x => x.category !== 'Status' && !x.damage && !x.damageCallback
          ).length * 8
        );
      case 'focussash':
        ...
        if (
          adjustedStats.hp < 65 &&
          adjustedStats.def < 65 &&
          adjustedStats.spd < 65
        )
          return 35;
        return 10;
      case 'heavydutyboots':
      ...
      case 'scopelens':
        ...
        if (moves.some(m => m.id === 'focusenergy')) {
          ...
          attacks.filter(x => (x.critRatio || 1) > 1).length ||
          ability === 'Super Luck'
        ) {
          return attacks.reduce((total, x) => {
            ...
            return (
              total + [0, 3, 6, 12][ratio] * (ability === 'Sniper' ? 4 / 3 : 1)
            );
          }, 0);
        }
        return 0;
      ...
      case 'flameorb':
        ...
        if (moves.some(m => m.id === 'facade')) {
          if (
            !weight &&
            !moves.some(
              m => TeamGenerator.moveIsPhysical(m, species) && m.id !== 'facade'
            )
          ) {
            weight = 30;
          } else {
      ...
      case 'blacksludge':
        ...
        return species.types.includes('Poison')
          ? moves.some(m => m.stallingMove)
            ? 20
            : 10
          : 0;
