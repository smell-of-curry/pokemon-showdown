...
export class Battle {
  ...
  constructor(options: BattleOptions) {
    ...
    if (this.format.battle) Object.assign(this, this.format.battle);

    this.id = '';
    this.debugMode = this.format.debug || !!options.debug;
    ...
    this.formatData = this.initEffectState({ id: this.format.id });
    this.gameType = this.format.gameType || 'singles';
    this.field = new Field(this);
    const isFourPlayer =
      this.gameType === 'multi' || this.gameType === 'freeforall';
    this.sides = Array(this.format.playerCount).fill(null) as any;
    this.activePerHalf =
      this.gameType === 'triples'
        ? 3
        : this.format.playerCount > 2 || this.gameType === 'doubles'
          ? 2
          : 1;
    this.prng = options.prng || new PRNG(options.seed || undefined);
    this.prngSeed = this.prng.startingSeed;
    this.rated = options.rated || !!options.rated;
    this.reportExactHP = !!this.format.debug;
  ...
  static compareRedirectOrder(a: AnyObject, b: AnyObject) {
    return (
      ...
      (a.effectHolder?.abilityState && b.effectHolder?.abilityState
        ? -(
            b.effectHolder.abilityState.effectOrder -
            a.effectHolder.abilityState.effectOrder
          )
        : 0) ||
      0
    );
  ...
  fieldEvent(eventid: string, targets?: Pokemon[]) {
    ...
    let handlers = this.findFieldEventHandlers(
      this.field,
      `onField${eventid}`,
      getKey
    );
    for (const side of this.sides) {
      if (side.n < 2 || !side.allySide) {
        handlers = handlers.concat(
          this.findSideEventHandlers(side, `onSide${eventid}`, getKey)
        );
      }
      for (const active of side.active) {
        ...
        if (eventid === 'SwitchIn') {
          handlers = handlers.concat(
            this.findPokemonEventHandlers(active, `onAny${eventid}`)
          );
        }
        if (targets && !targets.includes(active)) continue;
        handlers = handlers.concat(
          this.findPokemonEventHandlers(active, callbackName, getKey)
        );
        handlers = handlers.concat(
        ...
        handlers = handlers.concat(
          this.findBattleEventHandlers(callbackName, getKey, active)
        );
      }
    }
  ...
  runEvent(
    ...
    let effectSource: Pokemon | null = null;
  ...
  resolvePriority(h: EventListenerWithoutPriority, callbackName: string) {
    ...
    if (!handler.subOrder) {
      ...
      if (handler.effect.effectType === 'Condition') {
        ...
        if (
          handler.effect.name === 'Poison Touch' ||
    ...
    if (
      callbackName.endsWith('SwitchIn') ||
      callbackName.endsWith('RedirectTarget')
    ) {
    ...
    if (handler.effectHolder && (handler.effectHolder as Pokemon).getStat) {
      ...
      if (callbackName.endsWith('SwitchIn')) {
        ...
        const fieldPositionValue =
          pokemon.side.n * this.sides.length + pokemon.position;
        handler.speed -=
          this.speedOrder.indexOf(fieldPositionValue) /
    ...
    if (target instanceof Pokemon && (target.isActive || source?.isActive)) {
      ...
      if (prefixedHandlers) {
        for (const allyActive of target.alliesAndSelf()) {
          handlers.push(
            ...this.findPokemonEventHandlers(allyActive, `onAlly${eventName}`)
          );
          handlers.push(
            ...this.findPokemonEventHandlers(allyActive, `onAny${eventName}`)
          );
        }
        for (const foeActive of target.foes()) {
          handlers.push(
            ...this.findPokemonEventHandlers(foeActive, `onFoe${eventName}`)
          );
          handlers.push(
            ...this.findPokemonEventHandlers(foeActive, `onAny${eventName}`)
          );
        }
      }
      target = target.side;
  ...
  tiebreak() {
    ...
    this.add(
      ...
      tiedSides
        .map(
          (side, i) => `${side.name}: ${Math.round(hpTotal[i])} total HP left`
        )
        .join('; ')
    );
  ...
  win(side?: SideID | '' | Side | null) {
    ...
    } else if (!side || !this.sides.includes(side as Side)) {
      side = null;
    }
    side = side as Side | null;
    this.winner = side ? side.id : '';
        ...
        if (this.gen > 2) {
          for (const source of pokemon.foes()) {
            ...
            for (const abilitySlot in species.abilities) {
              const abilityName =
                species.abilities[abilitySlot as keyof Species['abilities']];
  ...
  maybeTriggerEndlessBattleClause(
    ...
    for (const side of this.sides) {
      ...
      for (const pokemon of side.pokemon) {
        berry = RESTORATIVE_BERRIES.has(toID(pokemon.pokemonSet.item));
        if (
          ['harvest', 'pickup'].includes(toID(pokemon.pokemonSet.ability)) ||
          pokemon.pokemonSet.moves.map(toID).includes('recycle' as ID)
        ) {
          cycle = true;
        }
  ...
  checkEVBalance() {
    ...
    for (const side of this.sides) {
      const sideLimitedEVs = !side.pokemon.some(
        pokemon =>
          Object.values(pokemon.pokemonSet.evs).reduce((a, b) => a + b, 0) > 510
      );
      if (limitedEVs === null) {
  ...
  spreadDamage(
    ...
    if (instafaint) {
      for (const [i, target] of targetArray.entries()) {
        ...
        if (target.hp <= 0) {
          this.debug(
            'instafaint: ' + this.faintQueue.map(entry => entry.target.name)
          );
    ...
    while (this.faintQueue.length) {
      ...
      if (
        ...
        if (pokemon.regressionForme) {
          ...
          pokemon.baseSpecies = this.dex.species.get(
            pokemon.pokemonSet.species || pokemon.pokemonSet.name
          );
          pokemon.baseAbility = toID(pokemon.pokemonSet.ability);
        }
  ...
  checkWin(faintData?: Battle['faintQueue'][0]) {
    const team1PokemonLeft =
      this.sides[0].pokemonLeft + (this.sides[0].allySide?.pokemonLeft || 0);
    const team2PokemonLeft =
      this.sides[1].pokemonLeft + (this.sides[1].allySide?.pokemonLeft || 0);
  ...
  runAction(action: Action) {
    ...
    for (const side of this.sides) {
      for (const pokemon of side.active) {
        if (pokemon && pokemon.forceSwitchFlag) {
    ...
    if (
      ...
      ['megaEvo', 'megaEvoX', 'megaEvoY'].includes(action.choice) &&
      this.gen === 7
    ) {
      this.eachEvent('Update');
  ...
  initEffectState(
    ...
    if (effectOrder !== undefined) {
      ...
      obj.id &&
      obj.target &&
      (!(obj.target instanceof Pokemon) || obj.target.isActive)
    ) {
