...
export class Battle {
	...
	constructor(options: BattleOptions) {
		...
		if (!!options.format && options.format.debug == undefined) {
			...
			this.format =
				options.format || Dex.formats.get(options.formatid, true);
		}
	...
	findPokemonEventHandlers(
		...
		if (callback !== undefined || (getKey && pokemon.abilityState[getKey])) {
			...
			if (
				callback !== undefined ||
				(getKey && pokemon.abilityState[getKey])
			) {
				handlers.push(
					this.resolvePriority(
						{
	...
	maybeTriggerEndlessBattleClause(
		...
		for (const side of this.sides) {
			...
			for (const pokemon of side.pokemon) {
				...
				if (
					["harvest", "pickup"].includes(
						toID(pokemon.pokemonSet.ability)
					) ||
	...
	checkEVBalance() {
		...
		for (const side of this.sides) {
			const sideLimitedEVs = !side.pokemon.some(
				(pokemon) =>
					Object.values(pokemon.pokemonSet.evs).reduce(
						(a, b) => a + b,
						0
					) > 510
			);
