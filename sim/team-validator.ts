  ...
  intersectWith(other: PokemonSources) {
    if (this.pomegEventEgg && other.pomegEggMoves) {
      ...
      for (const source of other.sources) {
        newSources.push(
          source.substr(0, 2) === '3E' ? this.pomegEventEgg : source
        );
      }
      ...
      for (const source of this.sources) {
        newSources.push(
          source.substr(0, 2) === '3E' ? other.pomegEventEgg : source
        );
      }
    ...
    if (this.sources.length) {
      if (other.sources.length) {
        ...
        const intersectSources = this.sources.filter(source =>
          sourcesSet.has(source)
        );
    ...
    if (other.learnsetDomain) {
      if (!this.learnsetDomain) {
        ...
        this.learnsetDomain.filter(source =>
          other.learnsetDomain?.includes(source)
        );
      }
    }
    if (this.possiblyLimitedEggMoves && !this.sourcesBefore) {
      const eggSources = this.sources.filter(
        source => source.charAt(1) === 'E'
      );
...
export class TeamValidator {
  ...
  constructor(format: string | Format, dex = Dex) {
    ...
    if (this.format.effectType !== 'Format') {
      throw new Error(
        `format should be a 'Format', but was a '${this.format.effectType}'`
      );
    }
  ...
  baseValidateTeam(
    ...
    for (const set of team) {
      ...
      if (options.skipSets && options.skipSets[set.name]) {
        ...
        setProblems = (format.validateSet || this.validateSet).call(
          this,
          set,
          teamHas
        );
      ...
      if (
        set.species === 'Pikachu-Starter' ||
        set.species === 'Eevee-Starter'
      ) {
        lgpeStarterCount++;
        if (lgpeStarterCount === 2 && ruleTable.isBanned('nonexistent')) {
          problems.push(
            `You can only have one of Pikachu-Starter or Eevee-Starter on a team.`
          );
        }
      }
      if (dex.gen === 3 && set.species.startsWith('Deoxys')) {
        if (!deoxysType) {
          ...
          deoxysType !== set.species &&
          ruleTable.isBanned('nonexistent')
        ) {
          return [
      ...
      if (options.removeNicknames) {
        const useCrossSpeciesNicknames =
          format.name.includes('Cross Evolution') ||
          ruleTable.has('franticfusionsmod');
        const species = dex.species.get(set.species);
        let crossSpecies: Species;
        if (
          useCrossSpeciesNicknames &&
          (crossSpecies = dex.species.get(set.name)).exists
        ) {
    ...
    for (const [rule, source, limit, bans] of ruleTable.complexTeamBans) {
      ...
      if (limit && count > limit) {
        ...
        const clause = source ? ` by ${source}` : ``;
        problems.push(
          `Your team has the combination of ${rule}, which is banned${clause}.`
        );
      }
    }
    ...
    if (format.onValidateTeam) {
      problems = problems.concat(
        format.onValidateTeam.call(this, team, format, teamHas) || []
      );
    }
  ...
  getEventOnlyData(
    ...
    if (!learnset.eventData) {
      throw new Error(
        `Event-only species ${species.name} has no eventData table`
      );
    }
  ...
  getValidationSpecies(set: PokemonSet): {
    ...
    if (
      ability.id === 'battlebond' &&
      toID(species.baseSpecies) === 'greninja'
    ) {
    ...
    if (ruleTable.has('obtainableformes')) {
      ...
      if (item.megaEvolves === species.name) {
        if (!item.megaStone)
          throw new Error(
            `Item ${item.name} has no base form for mega evolution`
          );
  ...
  validateSet(set: PokemonSet, teamHas: AnyObject): string[] | null {
    ...
    if (
      set.species.toLowerCase().endsWith('-gmax') &&
      this.format.id !== 'gen8megamax'
    ) {
    ...
    if (set.name && set.name.length > 18) {
      if (set.name === set.species) {
        ...
        problems.push(
          `Nickname "${set.name}" too long (should be 18 characters or fewer)`
        );
      }
    }
    ...
    if (
      ...
      (set.level === 100 && ruleTable.maxLevel < 100)
    ) {
    ...
    if (!set.evs)
      set.evs = TeamValidator.fillStats(
        null,
        ruleTable.evLimit === null ? 252 : 0
      );
    ...
    if (format.onChangeSet) {
      problems = problems.concat(
        format.onChangeSet.call(this, set, format, setHas, teamHas) || []
      );
    }
    ...
    if (set.hpType) {
      ...
      if (!type.exists || ['normal', 'fairy', 'stellar'].includes(type.id)) {
        problems.push(
          `${name}'s Hidden Power type (${set.hpType}) is invalid.`
        );
      } else {
    ...
    if (set.teraType) {
      ...
      if (
        dex.gen !== 9 ||
        (ruleTable.has('terastalclause') && !ruleTable.has('bonustypemod'))
      ) {
        delete set.teraType;
      }
    }
      ...
      if (dex.gen <= 2 || dex.currentMod === 'gen7letsgo') {
        ...
        if (ability.name === species.abilities['H']) {
          ...
          if (unreleasedHidden && ruleTable.has('-unreleased')) {
            ...
            problems.push(
              `${name}'s Hidden Ability is unreleased for the Orange and White forms.`
            );
          } else if (
            dex.gen === 5 &&
            set.level < 10 &&
            ...
            problems.push(
              `${name} must be at least level 10 to have a Hidden Ability.`
            );
          }
    ...
    if (!set.moves?.length) {
      problems.push(
        `${name} has no moves (it must have at least one to be usable).`
      );
      set.moves = [];
    }
    ...
    const pokemonGoProblems = this.validatePokemonGo(
      outOfBattleSpecies,
      set,
      setSources
    );
    if (ruleTable.isBanned('nonexistent')) {
      problems.push(
        ...this.validateStats(set, species, setSources, pokemonGoProblems)
      );
    }
    ...
    for (const moveName of set.moves) {
      ...
      if (problem) {
        let allowedByOM;
        if (
          problem.includes('hacking or glitches') &&
          ruleTable.has('omunobtainablemoves')
        ) {
          problem = `${name}'s ${problem}`;
          allowedByOM = !this.omCheckCanLearn(
            move,
            outOfBattleSpecies,
            setSources,
            set,
    ...
    if (
      (!setSources.sourcesBefore && setSources.sources.length) ||
      isUnderleveled
    ) {
      ...
      if (isUnderleveled && !setSources.sources.length) {
        let evoSpecies = species;
        while (evoSpecies.prevo) {
          const eventData = dex.species.getLearnsetData(
            evoSpecies.id
          ).eventData;
          if (eventData) {
            for (
              ...
              if (eventLevel && set.level >= eventLevel) {
                setSources.sources.push(
                  eventData[eventIndex].generation +
                    'S' +
                    eventIndex +
                    ' ' +
                    evoSpecies.id
      ...
      for (const source of setSources.sources) {
        if (isUnderleveled && source.charAt(1) !== 'S' && source !== '8V')
          continue;
      ...
      if (legalSources.length) {
        ...
        problems.push(
          `${name} must be at least level ${requiredLevel} to be evolved.`
        );
        const firstEventSource = setSources.sources.filter(
          source => source.charAt(1) === 'S'
        )[0];
        if (firstEventSource) {
        ...
        if (pokemonGoProblems && pokemonGoProblems.length) {
          problems.push(
            `It failed to validate as a Pokemon from Pokemon GO because:`
          );
      ...
      if (!legal) {
        if (
          !pokemonGoProblems ||
          (pokemonGoProblems && pokemonGoProblems.length)
        ) {
          ...
          for (const [i, event] of eventData.entries()) {
            if (
              ...
              event.generation >= this.minSourceGen
            ) {
    ...
    if (
      ruleTable.has('obtainablemisc') &&
      pokemonGoOnlySpecies.includes(species.id)
    ) {
      setSources.isFromPokemonGo = true;
      if (pokemonGoProblems && pokemonGoProblems.length) {
        problems.push(
    ...
    if (ruleTable.has('obtainablemoves')) {
      ...
      if (
        setSources.isHidden &&
        !canUseAbilityPatch &&
        setSources.maxSourceGen() < 5
      ) {
        problems.push(
          `${name} has a Hidden Ability - it can't use moves from before Gen 5.`
        );
      }
      if (
        ...
        problems.push(
          `${name} has an unbreedable Hidden Ability - it can't use egg moves.`
        );
      }
    }
    ...
    for (const [rule, source, limit, bans] of ruleTable.complexBans) {
      ...
      if (limit && count > limit) {
        ...
        if (source === 'Obtainable Moves') {
          ...
          problems.push(
            `${name} has the combination of ${rule}, which is banned${clause}.`
          );
        }
      }
    }
    ...
    if (
      nameSpecies.exists &&
      nameSpecies.name.toLowerCase() === set.name.toLowerCase()
    ) {
  ...
  validateStats(
    ...
    if (
      !set.hpType &&
      set.moves.some(m => dex.moves.get(m).id === 'hiddenpower')
    ) {
    ...
    const isLegendary =
      ...
      ['Manaphy', 'Cosmog', 'Cosmoem', 'Solgaleo', 'Lunala'].includes(
        species.baseSpecies
      );
    const diancieException = species.name === 'Diancie' && !set.shiny;
    const has3PerfectIVs =
      setSources.minSourceGen() >= 6 && isLegendary && !diancieException;
    ...
    if (dex.gen <= 2) {
      ...
      const expectedHpDV =
        (atkDV % 2) * 8 + (defDV % 2) * 4 + (speDV % 2) * 2 + (spcDV % 2);
      ...
      if (ivs.spa !== ivs.spd) {
        if (dex.gen === 2) {
          problems.push(
            `${name} has different SpA and SpD DVs, which is not possible in Gen 2.`
          );
        } else {
          ivs.spd = ivs.spa;
        }
      ...
      if (
        ...
        while (
          set.evs.atk > 0 &&
          2 * 80 + set.ivs.atk + Math.floor(set.evs.atk / 4) + 5 > 255
        ) {
          set.evs.atk -= 4;
        }
      }
    ...
    if (dex.currentMod === 'gen7letsgo') {
      ...
      for (const stat in set.evs) {
        if (set.evs[stat as 'hp'] > 0 && !allowAVs) {
          problems.push(
            `${name} has Awakening Values but this format doesn't allow them.`
          );
          break;
      ...
      for (const stat in set.evs) {
        if (set.evs[stat as StatID] > 255) {
          problems.push(
            `${name} has more than 255 EVs in ${Dex.stats.names[stat as 'hp']}.`
          );
        }
      }
      if (dex.gen <= 2) {
        if (set.evs.spa !== set.evs.spd) {
          if (dex.gen === 2) {
            problems.push(
              `${name} has different SpA and SpD EVs, which is not possible in Gen 2.`
            );
          } else {
    ...
    if (!this.format.debug) {
      if (set.level > 1 && evLimit !== 0 && totalEV === 0) {
        ...
        ![508, 510].includes(evLimit!) &&
        [508, 510].includes(totalEV)
      ) {
        problems.push(
  ...
  validateSource(
    ...
    if (source.charAt(1) === 'S') {
      ...
      const g7speciesName =
        species.gen > 2 && species.prevo ? species.prevo : species.id;
      const isHidden = !!this.dex.mod('gen7').species.get(g7speciesName)
        .abilities['H'];
      eventData = {
        generation: 2,
  ...
  findEggMoveFathers(
    ...
    if (eggGen === 3)
      eggMoves = eggMoves?.filter(
        eggMove => !setSources.pomegEggMoves?.includes(eggMove)
      );
    ...
    for (const father of dex.species.all()) {
      ...
      if (
        pokemonBlacklist.includes(father.id) &&
        !['dragonite', 'snorlax'].includes(father.id)
      )
        continue;
  ...
  fatherCanLearn(
    ...
    for (const move of moves) {
      ...
      for (const {
        learnset,
        species: curSpecies,
      } of this.dex.species.getFullLearnset(species.id)) {
        ...
        if (learnset[move]) {
          for (const moveSource of learnset[move]) {
            ...
            const canLearnFromSmeargle =
              moveSource.charAt(1) === 'E' && canBreedWithSmeargle;
            if (
              !'ESDV'.includes(moveSource.charAt(1)) ||
              canLearnFromSmeargle
            ) {
      ...
      const onlyEggSources = eggSources.sources.filter(
        source => source.charAt(1) === 'E'
      );
      if (
        eggGen >= 3 &&
        ...
        eggSources.possiblyLimitedEggMoves = [
          toID(eggSources.sourcesBefore + move),
        ];
      }
    ...
    pokemonBlacklist.push(species.id);
    if (
      allEggSources.limitedEggMoves &&
      allEggSources.limitedEggMoves.length > 1
    ) {
      ...
      if (
        !canChainbreed &&
        allEggSources.limitedEggMoves.length === moves.length
      )
        return false;
  ...
  validateForme(set: PokemonSet) {
    ...
    if (species.name === 'Necrozma-Ultra') {
      ...
      if (item.name !== 'Ultranecrozium Z') {
        ...
        problems.push(
          `Necrozma-Ultra must start the battle holding Ultranecrozium Z.`
        );
      ...
      if (
        ...
        !set.moves.map(toID).includes(toID(species.requiredMove))
      ) {
      ...
      if (
        item.forcedForme &&
        species.name === dex.species.get(item.forcedForme).baseSpecies
      ) {
    ...
    if (species.name in crowned) {
      const behemothMove = set.moves
        .map(toID)
        .indexOf(crowned[species.name] as ID);
      if (behemothMove >= 0) {
    ...
    if (
      species.baseSpecies === 'Rockruff' &&
      toID(set.ability) === 'owntempo'
    ) {
  ...
  checkSpecies(
    ...
    if (
      tierSpecies.baseSpecies === 'Rockruff' &&
      toID(set.ability) === 'owntempo'
    ) {
    ...
    const doublesTier =
      tierSpecies.doublesTier === '(DUU)' ? 'DNU' : tierSpecies.doublesTier;
    ...
    let nonexistentCheck =
      Tags.nonexistent.genericFilter!(tierSpecies) &&
      ruleTable.check('nonexistent');
  ...
  checkAbility(
    ...
    if (this.format.id.startsWith('gen9pokebilities')) {
      ...
      const unSeenAbilities = Object.keys(species.abilities)
        .filter(
          key => key !== 'S' && (key !== 'H' || !species.unreleasedHidden)
        )
  ...
  validateEvent(
    ...
    if (
      (eventData.shiny === true && !set.shiny) ||
      (!eventData.shiny && set.shiny)
    ) {
      if (fastReturn) return true;
    ...
    if (eventData.ivs) {
      ...
      const canBottleCap =
        dex.gen >= 7 && set.level >= (dex.gen < 9 ? 100 : 50);
      ...
      if (canBottleCap) {
        ...
        if (Object.keys(eventData.ivs).length >= 6) {
          const requiredHpType = dex.getHiddenPower(
            eventData.ivs as StatsTable
          ).type;
          if (set.hpType && set.hpType !== requiredHpType) {
            if (fastReturn) return true;
            problems.push(
              `${name} can only have Hidden Power ${requiredHpType}${etc}.`
            );
          }
    ...
    if (requiredIVs && set.ivs) {
      ...
      if (perfectIVs < requiredIVs) {
        ...
        if (eventData.perfectIVs) {
          problems.push(
            `${name} must have at least ${requiredIVs} perfect IVs${etc}.`
          );
        }
      }
    ...
    if (ruleTable.has('obtainablemoves')) {
      ...
      const tradebackEligible =
        dex.gen === 2 && (species.gen === 1 || eventSpecies.gen === 1);
      if (
        ssMaxSourceGen &&
        eventData.generation > ssMaxSourceGen &&
        !tradebackEligible
      ) {
        if (fastReturn) return true;
        problems.push(
          `${name} must not have moves only learnable in gen ${ssMaxSourceGen}${etc}.`
        );
      }

      if (
        eventData.from === 'Gen 5 Dream World' &&
        setSources.dreamWorldMoveCount > 1
      ) {
    ...
    if (ruleTable.has('obtainableabilities')) {
      if (
        ...
        if (species.name === eventSpecies.name) {
          ...
          if (ability1.gen && eventData.generation >= ability1.gen) {
            ...
            const requiredAbilitySlot =
              toID(eventData.abilities[0]) === ability1.id ? 1 : 0;
            const requiredAbility = dex.abilities.get(
              species.abilities[requiredAbilitySlot] || species.abilities['0']
            ).name;
            if (set.ability !== requiredAbility) {
              const originalAbility = dex.abilities.get(
                eventData.abilities[0]
              ).name;
              if (fastReturn) return true;
              problems.push(
      ...
      if (species.abilities['H']) {
        ...
        const canUseAbilityPatch =
          dex.gen >= 8 && this.format.mod !== 'gen8dlc1';
  ...
  validatePokemonGo(
    ...
    if (!pokemonGoData) {
      ...
      if (otherSpecies && !species.evoLevel) {
        const otherProblems = this.validatePokemonGo(
          otherSpecies,
          set,
          setSources,
          name
      ...
      if (set.shiny && pokemonGoSources.includes('noshiny')) {
        ...
        if (
          pokemonGoSources.includes('wild') &&
          !(set.shiny && pokemonGoSources.includes('nowildshiny'))
        ) {
          minLevel = 1;
          minIVs = 0;
        }
        ...
        if (!pokemonGoSources.includes('notrade')) {
          ...
          if (!set.shiny || species.id !== 'deoxys') {
            const specialTrade =
              pokemonGoSources.includes('specialtrade') || set.shiny;
        ...
        if (set.level < minLevel) {
          problems.push(
            `${name} must be at least level ${minLevel} to be from Pokemon GO.`
          );
        }
        ...
        if (hasEvenIVs) {
          problems.push(
            `${name} must have odd non-Speed IVs to be from Pokemon GO.`
          );
        }
        const canBottleCap =
          dex.gen >= 7 && set.level >= (dex.gen < 9 ? 100 : 50);
        if (
          ivs.atk !== ivs.spa &&
          !(canBottleCap && (ivs.atk === 31 || ivs.spa === 31))
        ) {
          problems.push(
            `${name}'s Atk and Sp. Atk IVs must match to be from Pokemon GO.`
          );
        }
        if (
          ivs.def !== ivs.spd &&
          !(canBottleCap && (ivs.def === 31 || ivs.spd === 31))
        ) {
          problems.push(
            `${name}'s Def and Sp. Def IVs must match to be from Pokemon GO.`
          );
        }
      }
    }
  ...
  omCheckCanLearn(
    ...
    const omVerdict = this.ruleTable.checkCanLearn[0].call(
      this,
      move,
      s,
      setSources,
  ...
  checkCanLearn(
    ...
    const canSketchPostGen7Moves =
      ruleTable.has('sketchpostgen7moves') ||
    ...
    for (const { species, learnset } of fullLearnset) {
      ...
      for (let learned of sources) {
        ...
        if (
          ...
          !setSources.learnsetDomain.includes(learnedGen + species.id) &&
        ...
        if (
          ...
          setSources.isHidden &&
          !canUseAbilityPatch &&
          !dex.mod('gen' + learnedGen).species.get(baseSpecies.name).abilities[
            'H'
          ]
        ) {
        ...
        if (species.isNonstandard !== 'CAP') {
          ...
          if (
            dex.gen >= 5 &&
            ['defog', 'whirlpool'].includes(moveid) &&
            learnedGen <= 4
          )
            blockedHM = true;
        }
        ...
        if (learned.charAt(1) === 'L') {
          ...
          if (level >= parseInt(learned.substr(2)) || learnedGen === 7) {
            ...
            if (
              species.gender === 'M' &&
              !this.motherCanLearn(toID(species.mother), moveid)
            ) {
        ...
        if (
          ...
          learned = (learnedGen +
            'E' +
            (species.prevo ? species.id : '')) as MoveSource;
          ...
          if (
            eventLearnset.eventData?.[parseInt(learned.charAt(2))]
              .emeraldEventEgg &&
            learnedGen === 3
          ) {
        ...
        if (
          learned.charAt(1) === 'E' &&
          learnedGen >= 8 &&
          !canLearnSpecies.includes(baseSpecies.id)
        ) {
          canLearnSpecies.push(baseSpecies.id);
        }
        if (!canLearnSpecies.includes(species.id))
          canLearnSpecies.push(species.id);
    ...
    const checkedSpecies = babyOnly
      ? fullLearnset[fullLearnset.length - 1].species
      : baseSpecies;
    if (
      checkedSpecies &&
      setSources.isFromPokemonGo &&
    ...
    if (!tradebackEligible) {
      if (!dex.species.getLearnsetData(nextSpecies.id).learnset) {
        nextSpecies = dex.species.get(
          nextSpecies.changesFrom || nextSpecies.baseSpecies
        );
      }
      while (nextSpecies) {
        for (let gen = nextSpecies.gen; gen <= dex.gen; gen++) {
           ...
           */
          if (
            canLearnSpecies.includes(nextSpecies.id) ||
            (0 < speciesCount && speciesCount < canLearnSpecies.length) ||
            (speciesCount === 0 && gen >= minLearnGen) ||
            (speciesCount === canLearnSpecies.length &&
              gen <= moveSources.sourcesBefore)
          ) {
    ...
    if (
      ...
      moveSources.possiblyLimitedEggMoves = [
        toID(moveSources.sourcesBefore + move.id),
      ];
    }
