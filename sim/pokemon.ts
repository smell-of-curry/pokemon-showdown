...
export class Pokemon {
	...
	getDetails = () => {
		...
		if (this.illusion) {
			const level = this.battle.ruleTable.has("illusionlevelmod")
				? this.illusion.level
				: this.level;
			...
			if (displayedSpeciesName === "Greninja-Bond")
				displayedSpeciesName = "Greninja";
			// COBBLED: uuid
			const illusionDetails =
				displayedSpeciesName +
				", " +
				this.illusion.uuid +
				(this.level === 100 ? "" : ", L" + this.level) +
				(this.illusion.gender === "" ? "" : ", " + this.illusion.gender) +
				(this.illusion.pokemonSet.shiny ? ", shiny" : "");
	...
	getStat(
		...
		if (
			statName === "spe" &&
			stat > 10000 &&
			!this.battle.format.battle?.trunc
		)
			stat = 10000;
		return stat;
	}
	...
	*/
		...
		for (const moveSlot of this.moveSlots) {
			...
			if (this.volatiles["dynamax"]) {
				...
				disabled =
					...
					(disabled &&
						canCauseStruggle.includes(moveSlot.disabledSource!));
			} else if (
			...
			moves: this[forAlly ? "baseMoves" : "moves"].map((move) => {
				if (move === "hiddenpower") {
					return (
						move +
						toID(this.hpType) +
						(this.battle.gen < 6 ? "" : this.hpPower)
					);
				}
			...
			if (this.volatiles[i].linkedPokemon) {
				...
				for (const linkedPoke of this.volatiles[i].linkedPokemon) {
					const linkedPokeLinks =
						linkedPoke.volatiles[this.volatiles[i].linkedStatus]
							.linkedPokemon;
	...
	formeChange(
		...
		source: Effect | null = this.battle.effect,
		isPermanent?: boolean, abilitySlot = '0',
		message?: string
	) {
	...
	eatItem(force?: boolean, source?: Pokemon, sourceEffect?: Effect) {
		...
		if (
			(!this.hp &&
				this.item !== "jabocaberry" &&
				this.item !== "rowapberry") ||
			!this.isActive
		)
			return false;
	...
	runImmunity(type: string, message?: string | boolean) {
		...
		const negateImmunity = !this.battle.runEvent(
			"NegateImmunity",
			this,
			type
		);
