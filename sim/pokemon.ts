...
export class Pokemon {
  ...
  constructor(pokemonSet: string | AnyObject, side: Side) {
    ...
    const pokemonScripts =
      this.battle.format.pokemon || this.battle.dex.data.Scripts.pokemon;
    if (pokemonScripts) Object.assign(this, pokemonScripts);

    if (typeof pokemonSet === 'string') pokemonSet = { name: pokemonSet };

    this.baseSpecies = this.battle.dex.species.get(
      pokemonSet.species || pokemonSet.name
    );
    ...
    for (const [i, moveid] of this.pokemonSet.moves.entries()) {
      let move = this.battle.dex.moves.get(moveid);
      if (!move.id) continue;
      if (move.id === 'hiddenpower' && move.type !== 'Normal') {
        if (!pokemonSet.hpType) pokemonSet.hpType = move.type;
      ...
      this.baseMoveSlots.push({
        ...
        pp: pokemonSet.movesInfo[i].pp,
        // COBBLED: Apply
        maxpp: pokemonSet.movesInfo[i].maxPp,
        target: move.target,
        disabled: false,
        disabledSource: '',
        used: false,
    ...
    this.abilityState = this.battle.initEffectState({
      id: this.ability,
      target: this,
    });

    this.item = toID(pokemonSet.item);
    this.itemState = this.battle.initEffectState({
      id: this.item,
      target: this,
    });
    this.lastItem = '';
    ...
    if (this.pokemonSet.currentHealth == 0 || this.pokemonSet.currentHealth) {
      this.hp = this.pokemonSet.currentHealth;
    } else {
      this.hp = this.maxhp;
    }
    // COBBLED: Apply status
    if (!!this.pokemonSet.status) {
      let status = this.battle.dex.conditions.get(this.pokemonSet.status);
      this.status = status.id;
      this.setStatus(status);
      if (this.pokemonSet.statusDuration !== -1) {
        this.statusState.duration = this.pokemonSet.statusDuration;
        this.statusState.startTime = this.pokemonSet.statusDuration;
        this.statusState.time = this.pokemonSet.statusDuration;
      }
    }

    // COBBLED: Apply faint status
    if (this.hp == 0) {
  ...
  getSlot(): PokemonSlot {
    const positionOffset =
      Math.floor(this.side.n / 2) * this.side.active.length;
  ...
  getFullDetails = () => {
    ...
    if (this.illusion) {
      details = this.illusion.getUpdatedDetails(
        this.battle.ruleTable.has('illusionlevelmod')
          ? this.illusion.level
          : this.level
      );
    }
  ...
  getStat(statName: StatIDExceptHP, unboosted?: boolean, unmodified?: boolean) {
    ...
    // stat boosts
    if (!unboosted) {
      const boosts = this.battle.runEvent('ModifyBoost', this, null, null, {
        ...this.boosts,
      });
    ...
    if (!unmodified) {
      ...
      stat = this.battle.runEvent(
        'Modify' + statTable[statName],
        this,
        null,
        null,
        stat
  ...
  getWeight() {
    const weighthg = this.battle.runEvent(
      'ModifyWeight',
      this,
      null,
      null,
  ...
  isAlly(pokemon: Pokemon | null) {
    return (
      !!pokemon &&
      (this.side === pokemon.side || this.side.allySide === pokemon.side)
    );
  }
  ...
  isAdjacent(pokemon2: Pokemon) {
    ...
    return (
      Math.abs(
        this.position + pokemon2.position + 1 - this.side.active.length
      ) <= 1
    );
  }
  ...
  getAtLoc(targetLoc: number) {
    let side =
      this.battle.sides[
        targetLoc < 0 ? this.side.n % 2 : (this.side.n + 1) % 2
      ];
  ...
  getLocOf(target: Pokemon) {
    const positionOffset =
      Math.floor(target.side.n / 2) * target.side.active.length;
  ...
  getMoves(
    ...
    for (const moveSlot of this.moveSlots) {
      ...
      if (this.volatiles['dynamax']) {
        ...
        (disabled &&
          this.side.active.length >= 2 &&
          this.battle.actions.targetTypeChoices(target!))
      ) {
        disabled = true;
      }
  ...
  getSwitchRequestData(forAlly?: boolean) {
    const entry: ISidePokemonRequest = {
      ...
      moves: this[forAlly ? 'baseMoves' : 'moves'].map(move => {
        if (move === 'hiddenpower') {
          return (
            move + toID(this.hpType) + (this.battle.gen < 6 ? '' : this.hpPower)
          );
        }
        if (move === 'frustration' || move === 'return') {
          const basePowerCallback = this.battle.dex.moves.get(move)
            .basePowerCallback as (pokemon: Pokemon) => number;
    ...
    if (this.battle.gen >= 9) {
      ...
      entry.reviving =
        this.isActive &&
        !!this.side.slotConditions[this.position]['revivalblessing'];
    }
    if (this.battle.gen === 9) {
  ...
  getCappedBoost(boosts: SparseBoostsTable) {
    ...
    for (boostName in boosts) {
      ...
      cappedBoost[boostName] =
        this.battle.clampIntRange(this.boosts[boostName] + boost, -6, 6) -
  ...
  copyVolatileFrom(pokemon: Pokemon, switchCause?: string | boolean) {
    ...
    for (const i in pokemon.volatiles) {
      ...
      // shallow clones
      this.volatiles[i] = this.battle.initEffectState({
        ...pokemon.volatiles[i],
      });
  ...
  transformInto(pokemon: Pokemon, effect?: Effect) {
    ...
    this.setType(
      pokemon.volatiles['roost'] ? pokemon.volatiles['roost'].typeWas : types,
      true
    );
      ...
      for (const volatile of volatilesToCopy) {
        if (pokemon.volatiles[volatile]) {
          ...
          if (volatile === 'dragoncheer')
            this.volatiles[volatile].hasDragonType =
    ...
    if (
      ['Ogerpon', 'Terapagos'].includes(this.species.baseSpecies) &&
      this.canTerastallize
    )
  ...
  setSpecies(
    ...
    const species = this.battle.runEvent(
      'ModifySpecies',
      this,
      null,
      source,
    ...
    const stats = this.battle.spreadModify(
      this.species.baseStats,
      this.pokemonSet
    );
  ...
  formeChange(
    ...
    if (isPermanent) {
      ...
      this.battle.add(
        ...
        this,
        details,
        source ? `[from] ${source.name}` : undefined
      );
    } else {
  ...
  clearVolatile(includeSwitchFlags = true) {
    ...
    if (
      this.battle.gen === 1 &&
      this.baseMoves.includes('mimic' as ID) &&
      !this.transformed
    ) {
      const moveslot = this.baseMoves.indexOf('mimic' as ID);
      const mimicPP = this.moveSlots[moveslot]
        ? this.moveSlots[moveslot].pp
        : 16;
    ...
    for (const i in this.volatiles) {
      if (this.volatiles[i].linkedStatus) {
        this.removeLinkedVolatiles(
          this.volatiles[i].linkedStatus,
          this.volatiles[i].linkedPokemon
        );
      }
    }
    if (
      this.species.name === 'Eternatus-Eternamax' &&
      this.volatiles['dynamax']
    ) {
  ...
  eatItem(force?: boolean, source?: Pokemon, sourceEffect?: Effect) {
    ...
    if (
      (!this.hp && this.item !== 'jabocaberry' && this.item !== 'rowapberry') ||
      !this.isActive
    )
      return false;
    ...
    const item = this.getItem();
    if (
      sourceEffect?.effectType === 'Item' &&
      this.item !== sourceEffect.id &&
      source === this
    ) {
  ...
  getNature() {
    return this.battle.dex.natures.get(this.pokemonSet.nature);
  }
  ...
  removeLinkedVolatiles(
    linkedStatus: string | Effect,
    ...
    for (const linkedPoke of linkedPokemon) {
      ...
      volatileData.linkedPokemon.splice(
        volatileData.linkedPokemon.indexOf(this),
        1
      );
  ...
  isSkyDropped() {
    ...
    for (const foeActive of this.side.foe.active) {
      if (
        foeActive.volatiles['skydrop'] &&
        foeActive.volatiles['skydrop'].source === this
      ) {
        return true;
      }
    }
  ...
  runEffectiveness(move: ActiveMove) {
    ...
    if (this.terastallized && move.type === 'Stellar') {
      ...
      for (const type of this.getTypes()) {
        ...
        typeMod = this.battle.singleEvent(
          'Effectiveness',
          move,
          null,
          this,
        ...
        totalTypeMod += this.battle.runEvent(
          'Effectiveness',
          this,
          type,
          move,
