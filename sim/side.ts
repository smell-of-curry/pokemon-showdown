...
export class Side {
	...
	canDynamaxNow(): boolean {
		...
		if (
			...
			this.battle.turn % 2 !== [1, 1, 0, 0][this.n]
		)
			return false;
	...
	foes(all?: boolean) {
		if (this.battle.gameType === "freeforall") {
			return this.battle.sides
				...
				.filter(
					(pokemon) =>
						pokemon && pokemon.side !== this && (all || !!pokemon.hp)
				);
		}
	...
	activeTeam() {
		...
		return this.battle.sides[this.n % 2].active.concat(
			this.battle.sides[(this.n % 2) + 2].active
		);
	}
		...
		if (!pokemon)
			throw new Error(
				`Could not find pokemon at targetLoc: ${targetLoc}, ${index}: ${
					...
					Object.values(this.pokemon).map(
						(v, i) => `${i}: ${v.species.name}`
					)
				)}`
			);

		this.battle.useBagItem(pokemon, itemId);
	}
	...
	chooseMove(
		...
		if (
			typeof moveText === "number" ||
			(moveText && /^[0-9]+$/.test(moveText))
		) {
			// Parse a one-based move index.
			const moveIndex = Number(moveText) - 1;
			if (
				moveIndex < 0 ||
				moveIndex >= request.moves.length ||
				!request.moves[moveIndex]
			) {
				return this.emitChoiceError(
					`Can't move: Your ${pokemon.name} doesn't have a move ${
						moveIndex + 1
					}`
				);
			}
			...
			if (
				!targetType &&
				["", "dynamax"].includes(event) &&
				request.maxMoves
			) {
				for (const [
					i,
					moveRequest,
				] of request.maxMoves.maxMoves.entries()) {
		...
		const zMove =
			event === "zmove"
				? this.battle.actions.getZMove(move, pokemon)
				: undefined;
		...
		if (lockedMove) {
			...
			if (
				pokemon.volatiles[lockedMoveID] &&
				pokemon.volatiles[lockedMoveID].targetLoc
			) {
			...
			if (!isEnabled) {
				...
				const includeRequest = this.updateRequestForPokemon(
					pokemon,
					(req) => {
						let updated = false;
		...
		const canDynamax =
			this.activeRequest?.active[this.active.indexOf(pokemon)].canDynamax;
	...
	updateRequestForPokemon(
		pokemon: Pokemon,
		update: (req: AnyObject) => boolean
	) {
	...
	chooseSwitch(slotText?: string) {
		...
		if (slot >= this.pokemon.length) {
			return this.emitChoiceError(
				`Can't switch: You do not have a Pokémon in slot ${
					slot + 1
				} to switch to`
			);
		} else if (
			...
			return this.emitChoiceError(
				`Can't switch: The Pokémon in slot ${
					slot + 1
				} can only switch in once`
			);
		}
	...
	pickedTeamSize() {
		return Math.min(
			this.pokemon.length,
			this.battle.ruleTable.pickedTeamSize || Infinity
		);
	}
	...
	chooseTeam(data = "") {
		...
		for (const [index, pos] of positions.entries()) {
			if (isNaN(pos) || pos < 0 || pos >= this.pokemon.length) {
				return this.emitChoiceError(
					`Can't choose for Team Preview: You do not have a Pokémon in slot ${
						pos + 1
					}`
				);
			}
			if (positions.indexOf(pos) !== index) {
				return this.emitChoiceError(
					`Can't choose for Team Preview: The Pokémon in slot ${
						pos + 1
					} can only switch in once`
				);
			}
		...
		if (ruleTable.valueRules.has("forceselect")) {
			const species = this.battle.dex.species.get(
				ruleTable.valueRules.get("forceselect")
			);
			if (!data) {
				// autoChoose
				positions = [...this.pokemon.keys()]
					...
					.concat(
						[...this.pokemon.keys()].filter(
							(pos) => this.pokemon[pos].species.name !== species.name
						)
					)
					.slice(0, pickedTeamSize);
			} else {
				...
				if (!hasSelection) {
					return this.emitChoiceError(
						`You must bring ${species.name} to the battle.`
					);
				}
			}
		}
	...
	clearChoice() {
		...
		if (this.battle.requestState === "switch") {
			const canSwitchOut = this.active.filter(
	...
	choose(input: string) {
		...
		const choiceStrings = input.startsWith("team ")
			? [input]
			: input.split(",");
		...
		if (this.requestState === "teampreview") {
			...
			while (!this.isChoiceDone()) {
				if (!this.chooseSwitch())
					throw new Error(
						`autoChoose switch crashed: ${this.choice.error}`
					);
				i++;
