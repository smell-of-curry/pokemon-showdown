...
export class Side {
      ...
      .map(action => {
        switch (action.choice) {
          case 'move':
            ...
            if (action.mega)
              details +=
                action.pokemon!.item === 'ultranecroziumz' ? ` ultra` : ` mega`;
  ...
  foes(all?: boolean) {
    if (this.battle.gameType === 'freeforall') {
      return this.battle.sides
        ...
        .filter(
          pokemon => pokemon && pokemon.side !== this && (all || !!pokemon.hp)
        );
    }
  ...
  addSideCondition(
    ...
    this.sideConditions[status.id] = this.battle.initEffectState({
      ...
      target: this,
      source,
      sourceSlot: (source as Pokemon).getSlot(),
