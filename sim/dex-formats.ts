...
export class RuleTable extends Map<string, string> {
  ...
  resolveNumbers(format: Format, dex: ModdedDex) {
    ...
    if (
      timer.starting !== undefined &&
      (timer.starting < 10 || timer.starting > 1200)
    ) {
      throw new Error(
    ...
    if (
      timer.maxPerTurn !== undefined &&
      (timer.maxPerTurn < 10 || timer.maxPerTurn > 1200)
    ) {
      throw new Error(
...
export class Format extends BasicEffect implements Readonly<BasicEffect> {
  ...
  constructor(data: AnyObject) {
    ...
    this.effectType =
      (Utils.getString(data.effectType) as FormatEffectType) || 'Condition';
...
export class DexFormats {
  ...
  load(): this {
    ...
    if (customFormats)
      Formats = mergeFormatLists(Formats as any, customFormats);
  ...
  getRuleTable(
    ...
    if (format.name.length > 50) {
      throw new Error(
        `Format "${format.name}" has a name longer than 50 characters`
      );
    }
    if (depth === 1) {
    ...
    for (const rule of ruleset) {
      ...
      if (rule.startsWith('!') && !rule.startsWith('!!')) {
        ...
        if (repealDepth === depth) {
          throw new Error(
            `Rule "${rule}" did nothing because "${rule.slice(1)}" is not in effect`
          );
        }
      ...
      if ('+*-'.includes(ruleSpec.charAt(0))) {
        if (ruleTable.has(ruleSpec)) {
          throw new Error(
            `Rule "${rule}" in "${format.name}" already exists in "${ruleTable.get(ruleSpec) || format.name}"`
          );
        }
        for (const prefix of '+*-')
      ...
      if (subformat.hasValue) {
        ...
        if (
          (subformat.id === 'pickedteamsize' || subformat.id === 'evlimit') &&
          value === 'Auto'
        ) {
        ...
        if (oldValue === value) {
          ...
          if (oldValue === undefined) {
            if (
              ...
              if (
                this.dex.formats.get(subformat.mutuallyExclusiveWith).ruleset
                  .length
              ) {
        ...
        if (ruleTable.has(subformat.id) && !repealAndReplace) {
          throw new Error(
            `Rule "${rule}" in "${format.name}" already exists in "${ruleTable.get(subformat.id) || format.name}"`
          );
        }
      }
