...
export const State = new (class {
  ...
  serializeBattle(battle: Battle): /* Battle */ AnyObject {
    const state: /* Battle */ AnyObject = this.serialize(
      battle,
      BATTLE,
      battle
    );
  ...
  deserializeBattle(serialized: string | /* Battle */ AnyObject): Battle {
    ...
    if (activeRequests) {
      ...
      for (const [i, side] of state.sides.entries()) {
        battle.sides[i].activeRequest =
          side.activeRequest === null ? null : requests[i];
      }
    }
  ...
  serializePokemon(pokemon: Pokemon): /* Pokemon */ AnyObject {
    const state: /* Pokemon */ AnyObject = this.serialize(
      pokemon,
      POKEMON,
      pokemon.battle
    );
    ...
    if (
      ...
      state.baseMoveSlots = this.serializeWithRefs(
  ...
  deserializePokemon(state: /* Pokemon */ AnyObject, pokemon: Pokemon) {
    ...
    if (state.baseMoveSlots) {
      baseMoveSlots = this.deserializeWithRefs(
        state.baseMoveSlots,
        pokemon.battle
      );
  ...
  serializeChoice(choice: Choice, battle: Battle): /* Choice */ AnyObject {
    const state: /* Choice */ AnyObject = this.serialize(
      choice,
      CHOICE,
      battle
    );
  ...
  isActiveMove(obj: AnyObject): obj is ActiveMove {
    return (
      obj.hasOwnProperty('hit') &&
      (obj.hasOwnProperty('id') || obj.hasOwnProperty('move'))
    );
  }
  ...
  serializeActiveMove(
    ...
    const state: /* ActiveMove */ AnyObject = this.serialize(
      move,
      skip,
      battle
    );
  ...
  deserializeActiveMove(
    state: /* ActiveMove */ AnyObject,
    battle: Battle
  ): ActiveMove {
    const move = battle.dex.getActiveMove(
      this.fromRef(state.move, battle)! as Move
    );
    ...
    switch (typeof obj) {
      ...
      case 'object':
        ...
        if (obj.constructor !== Object) {
          ...
          throw new TypeError(
            `Unsupported type ${obj.constructor.name}: ${obj}`
          );
        }
  ...
  toRef(obj: Referable): string {
    ...
    const id =
      obj instanceof Pokemon
        ? `${obj.side.id}${POSITIONS[obj.position]}`
        : `${obj.id}`;
  ...
  fromRef(ref: string, battle: Battle): Referable | undefined {
    ...
    switch (type) {
      ...
      case 'Pokemon':
        return battle.sides[Number(id[1]) - 1].pokemon[
          POSITIONS.indexOf(id[2])
        ];
      case 'Ability':
