...
export class Runner {
  static readonly AI_OPTIONS: AIOptions = {
    createAI: (s: ObjectReadWriteStream<string>, o: AIOptions) =>
      new RandomPlayerAI(s, o),
    move: 0.7,
    mega: 0.6,
  };
  ...
  private async runGame(
    format: string,
    battleStream: RawBattleStream | DualStream
  ) {
    ...
    if (is4P) {
      p3 = this.p4options.createAI(streams.p3, {
        seed: this.newSeed(),
        ...this.p3options,
      });
      p4 = this.p4options.createAI(streams.p4, {
        seed: this.newSeed(),
        ...this.p4options,
      });
    }
...
class DualStream {
  ...
  async read() {
    ...
    if (!this.debug)
      assert.equal(State.normalizeLog(test), State.normalizeLog(control));
    return control;
  }
