...
export class RandomPlayerAI extends BattlePlayer {
  ...
  protected handleMoveRequest(request: IShowdownRequest): string {
    ...
    const commands = activeSlots.map((active, i) => {
      ...
      if (myPokemon[i].condition.endsWith(' fnt') || myPokemon[i].commanding) {
        return 'pass';
      }
      ...
      const usingMaxMoves =
        (!active.canDynamax && active.maxMoves) || (doTransform && canDynamax);
      const rawMoves = usingMaxMoves ? active.maxMoves!.maxMoves : active.moves;
      ...
      const hasAlly =
        myPokemon.length > 1 && !myPokemon[i ^ 1].condition.endsWith(' fnt');
      ...
      const moveChoices = finalMoves.map(mOpt => {
        ...
        if (request.active && request.active.length > 1) {
          if (['normal', 'any', 'adjacentFoe'].includes(mOpt.target ?? '')) {
