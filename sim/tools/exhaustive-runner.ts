...
export class ExhaustiveRunner {
    ...
    do {
      ...
      try {
        ...
        await new Runner({
          ...
          p3options: is4P
            ? { team: generator.generate(), createAI }
            : undefined,
          p4options: is4P
            ? { team: generator.generate(), createAI }
            : undefined,
  ...
  private createPools(dex: typeof Dex): Pools {
    return {
      ...
      items: new Pool(
        ExhaustiveRunner.onlyValid(dex.gen, dex.data.Items, i =>
          dex.items.get(i)
        ),
        this.prng
      ),
      abilities: new Pool(
        ExhaustiveRunner.onlyValid(dex.gen, dex.data.Abilities, a =>
          dex.abilities.get(a)
        ),
        this.prng
      ),
      moves: new Pool(
        ExhaustiveRunner.onlyValid(
          ...
          m =>
            m !== 'struggle' &&
            (m === 'hiddenpower' || m.substr(0, 11) !== 'hiddenpower')
        ),
        this.prng
      ),
    };
...
class TeamGenerator {
  ...
  get exhausted() {
    const exhausted = [
      this.pools.pokemon.exhausted,
      this.pools.moves.exhausted,
    ];
  ...
  private choosePokemon(choices: { slot: number; pokemon: AnyObject }[]) {
    ...
    for (const { slot, pokemon } of choices) {
      ...
      if (
        ...
        pokemon.moves.some(
          (m: AnyObject) => !this.pools.moves.wasUsed(this.fixMove(m))
        )
      ) {
