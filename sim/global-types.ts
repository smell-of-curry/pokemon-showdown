...
type EffectState = import('./pokemon').EffectState;
...
interface ModdedBattlePokemon {
  ...
  isGrounded?: (
    this: Pokemon,
    negateImmunity: boolean | undefined
  ) => boolean | null;
  modifyStat?: (
    this: Pokemon,
    statName: StatIDExceptHP,
...
interface IShowdownRequest {
  /**
   * Specifies if the player should wait for the next request.
   * If this is set/true `forceSwitch` or `active` will not be present.
   */
  wait?: boolean;
  /**
   * Is defined if the player is forced to switch
   * each index in the array represents a index in {@link active}
   * `true` represents that the player must switch out that pokemon, false means its fine.
   */
  forceSwitch?: boolean[];
  /**
   * Represents the active pokemon of the player
   */
  active?: IActivePokemonRequest[];
  /**
   * Represents the side of the player, this includes all the pokemon and identity of the player
   */
  side: {
    name: string;
    id: SideID;
