...
export class BattleStream extends Streams.ObjectReadWriteStream<string> {
  ...
  _write(chunk: string, throwError = false) {
    try {
      if (this.noCatch) {
  ...
  _writeLine(type: string, message: string) {
    switch (type) {
      ...
      case 'forceforme':
        ...
        const [position, species, details, hpString, source] =
          message.split('|');
        ...
        if (pokemonPosition.length >= 3) {
          ...
          if (
            ...
            if (side && side.active[posIndex]) {
              battlePokemon = side.active[posIndex];
            }
          }
        }
        ...
        if (battlePokemon) {
          ...
          try {
            ...
            console.warn('Error in forceforme: ' + err);
          }
        }
        break;
      case 'forcewin':
