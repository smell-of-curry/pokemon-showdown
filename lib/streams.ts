...
export class ReadStream {
  ...
  constructor(
    ...
    if (typeof optionsOrStreamLike === 'string') {
      ...
      typeof (optionsOrStreamLike as any)._readableState === 'object'
    ) {
  ...
  peek(
    byteCount: number | string | null = null,
    ...
    if (maybeLoad)
      return maybeLoad.then(() => this.peek(byteCount as number, encoding));
  ...
  async read(
    ...
    if (out && typeof out !== 'string') {
      throw new Error(
        'Race condition; you must not read before a previous read has completed'
      );
    }
...
export class WriteStream {
  ...
  constructor(
    optionsOrStream: WriteStreamOptions | NodeJS.WritableStream = {}
  ) {
...
export class ObjectReadStream<T> {
  ...
  async pipeTo(
    outStream: ObjectWriteStream<T>,
    options: { noEnd?: boolean } = {}
  ) {
    let value, done;
    while ((({ value, done } = await this.next()), !done)) {
      await outStream.write(value!);
...
export class ObjectWriteStream<T> {
  ...
  constructor(
    optionsOrStream: ObjectWriteStreamOptions<T> | NodeJS.WritableStream = {}
  ) {
    this.isReadable = false;
    this.isWritable = true;
    this.nodeWritableStream = null;
    ...
    if (options.nodeStream) {
      ...
      options.write = function (data: T) {
        const result = this.nodeWritableStream!.write(
...
interface ObjectReadWriteStreamOptions<T> {
  ...
  destroy?: (this: ObjectReadStream<T>) => void | Promise<void>;
  write?: (
    this: ObjectWriteStream<T>,
    elem: T
  ) => Promise<any> | undefined | void;
  writeEnd?: () => Promise<any> | undefined | void;
}

export class ObjectReadWriteStream<T>
  extends ObjectReadStream<T>
  implements ObjectWriteStream<T>
{
  isReadable: true;
  isWritable: true;
  nodeWritableStream: NodeJS.WritableStream | null;
