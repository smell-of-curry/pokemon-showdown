...
export const Formats: import('../sim/dex-formats').FormatList = [
  ...
  },
  {
    ...
    onBegin() {
      ...
      this.add(
        '-message',
        `You can find our thread and metagame resources here:`
      );
  ...
  },
  {
    ...
    getSharedPower(pokemon) {
      ...
      for (const ally of pokemon.side.pokemon) {
        if (
          pokemon.battle.ruleTable.isRestricted(`ability:${ally.baseAbility}`)
        )
          continue;
  ...
  },
  {
    ...
    onValidateSet(set) {
      ...
      if (this.ruleTable.isRestricted(`move:${fsMove.id}`)) {
        return [
          `${set.name}'s move ${fsMove.name} cannot be used as a future move.`,
        ];
      }
    },
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      if (!teamHas.abilityMap) {
        ...
        for (const pokemon of Dex.species.all()) {
          if (
            ...
            !this.ruleTable.has(
              `+pokemontag:${this.toID(pokemon.isNonstandard)}`
            )
          )
            continue;
          ...
          for (const key of Object.values(pokemon.abilities)) {
            ...
            if (abilityId in teamHas.abilityMap) {
              teamHas.abilityMap[abilityId][pokemon.evos ? 'push' : 'unshift'](
                pokemon.id
              );
            } else {
      ...
      for (const donor of pokemonWithAbility) {
        ...
        if (
          annoyingPokemon.includes(donorSpecies.name) ||
          annoyingPokemon.includes(species.name)
        ) {
          set.hpType = 'Dark';
        }
        const problems = this.validateSet(set, teamHas);
      ...
      spreadModify(baseStats, set) {
        ...
        for (statName in modStats) {
          ...
          modStats[statName] = tr(
            (tr(2 * stat + set.ivs[usedStat] + tr(set.evs[usedStat] / 4)) *
              set.level) /
              100 +
              5
          );
    ...
    onBegin() {
      for (const pokemon of this.getAllPokemon()) {
        ...
        pokemon.m.innates = Object.keys(pokemon.species.abilities)
          .filter(
            key =>
              key !== 'S' && (key !== 'H' || !pokemon.species.unreleasedHidden)
          )
          .map(key =>
            this.toID(pokemon.species.abilities[key as '0' | '1' | 'H' | 'S'])
          )
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      if (set.moves?.length) {
        ...
        for (const [i, moveid] of set.moves.entries()) {
          ...
          if (
            ...
            !(
              this.ruleTable.has(
                `+pokemontag:${this.toID(pokemove.isNonstandard)}`
              ) ||
              this.ruleTable.has(`+pokemon:${pokemove.id}`) ||
              this.ruleTable.has(
                `+basepokemon:${this.toID(pokemove.baseSpecies)}`
              )
            )
          ) {
            problems.push(
          ...
          if (
            ...
            problems.push(
              `${pokemove.name} is unable to be used as a Pokemove.`
            );
          }
          pokemoves++;
          moves.push(moveid);
      ...
      const allowedPokemoves =
        this.ruleTable.valueRules.get('allowedpokemoves') || 1;
  ...
  },
  {
    ...
    onValidateRule() {
      if (this.dex.gen !== 9) {
        throw new Error(
          `Tera Donation is not supported in generations without terastallization.`
        );
      }
    },
    onSwitchIn(pokemon) {
    ...
    actions: {
      modifyDamage(baseDamage, pokemon, target, move, suppressMessages) {
        ...
        // COBBLED: Add source pokemon to the crit message
        if (isCrit && !suppressMessages)
          this.battle.add('-crit', target, pokemon);
  ...
  },
  {
    ...
    validateSet(set, teamHas) {
      ...
      if (!teamHas.trademarks)
        teamHas.trademarks = new this.dex.Multiset<string>();
      ...
      if (set.item) {
        ...
        if (
          ...
          !(
            ...
            this.ruleTable.has(`+pokemontag:mega`)
          )
        ) {
  ...
  },
  {
    ...
    onBegin() {
      for (const pokemon of this.getAllPokemon()) {
        if (!(pokemon.pokemonSet as any).teraCaptain)
          pokemon.canTerastallize = null;
      }
      this.add(
        'rule',
        'Tera Captain Clause: Only Tera Captains can Terastallize'
      );
    },
    onTeamPreview() {
      this.add('clearpoke');
